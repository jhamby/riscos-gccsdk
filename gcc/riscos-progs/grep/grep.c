     ...
     struct stats *stats;
{
  ...
  if (! name_space)
      ...
      while (*namep)
	{
	  ...
	  fprintf (stderr, "namep ='%s', namelen=%d, file=%x, dirlen=%d\n",
	  	  namep,namelen, file, dirlen);
	  if (dirlen + 1 + namelen + 1 > filelen)
	    {
	      filelen = dirlen + 1 + namelen + 1 + 32;
	      file = xrealloc (file, filelen);
	    }
	  strcpy (file, dir);
	  file[dirlen] = '/';
