     ...
     int depth;
{
  ...
  for (i = 0; i <= 1; i++)
      ...
      if (inf[i].desc != -1)
	{
	  ...
	  if (i && filename_cmp (inf[i].name, inf[0].name) == 0)
	    ...
	    stat_result = stat ((char *)inf[i].name, &inf[i].stat);
  ...
  if (! failed && depth == 0 && inf[0].dir_p != inf[1].dir_p)
	...
	fatal ("can't compare - to a directory");
      ...
      if (stat ((char *)filename, &inf[dir_arg].stat) != 0)
	{
  ...
  if (failed)
  ...
  else
    {
      ...
      if (inf[0].desc == -2)
	if ((inf[0].desc = open ((char *)inf[0].name, O_RDONLY, 0)) < 0)
	  {
	...
	else if ((inf[1].desc = open ((char *)inf[1].name, O_RDONLY, 0)) < 0)
	  {
