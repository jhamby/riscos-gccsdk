...
#if TRUE
    /* Heap4 versions of the ANSI C memory allocation functions */
/*extern void *HDmalloc(size_t size, const char *file, unsigned long line);
extern void *HDrealloc(void *oldptr, size_t size, const char *file, unsigned long line);
extern void *HDcalloc(size_t count, size_t size, const char *file, unsigned long line);
extern void  HDfree(void *block, const char *file, unsigned long line);
extern char *HDstrdup(const char *s, const char *file, unsigned long line);

    #define malloc(size)           HDmalloc(size, __FILE__, __LINE__)
    #define realloc(ptr,new_size)  HDrealloc(ptr, new_size, __FILE__, __LINE__)
    #define calloc(num,size)       HDcalloc(num, size, __FILE__, __LINE__)
    #define free(ptr)              HDfree(ptr, __FILE__, __LINE__)
    #define strdup(ptr)            HDstrdup(ptr, __FILE__, __LINE__)*/
#endif
