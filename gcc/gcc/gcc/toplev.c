     ...
     va_list ap;
{
  ...
  ERROR_THROWBACK (file, line, (warn) ? "warning" : NULL, msgid, ap);
#endif
  report_file_and_line (file, line, warn);
  vnotice (stderr, msgid, ap);
  fputc ('\n', stderr);
     ...
     va_list ap;
{
  ...
  fputc ('\n', stderr);
#ifdef ERROR_THROWBACK
  ERROR_THROWBACK (DECL_SOURCE_FILE (decl), DECL_SOURCE_LINE (decl),
                   (warn) ? "warning" : NULL, msg, 0);
#endif
}
...
#ifdef ERROR_THROWBACK_INITIALISE
  ERROR_THROWBACK_INITIALISE;
#endif

  compile_file (filename);

#ifdef ERROR_THROWBACK_FINALISE
  ERROR_THROWBACK_FINALISE;
#endif
