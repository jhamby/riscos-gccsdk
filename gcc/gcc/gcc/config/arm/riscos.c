  ...
  if (arm_throwback_started > 0 && riscos_canonicalise_filename (file))
    {
      ...
      /* Errors generated by the C++ compiler can stretch over multiple
	 lines.  DDEUtils uses the newline as a string terminator.  We
	 must call arm_throwback_error for as many times as there are
	 newlines.  */
      while (*p != '\0')
	{
