...
static int arm_leaf_function_p PROTO ((void));
static int arm_stack_check_needed PROTO ((void));
...
/* Set to one if the current function is a leaf function. Set to zero if
   it isn't. Set to -1 if the function type hasn't been decided.  */
static int arm_leaf_function = -1;
     ...
     int iscond;
{
  ...
  if (!reload_completed
      ...
      || (current_function_calls_alloca && ! TARGET_APCS)
      || (current_function_outgoing_args_size && ! TARGET_APCS)
      || (get_frame_size () && ! arm_apcs_frame_needed ()))
    return 0;
     ...
     rtx scan;
{
  ...
  for (i = 0; i < pool_size; i++)
	...
	default:
	  fprintf (stderr, "dump_table: size = %d\n",
	  	   GET_MODE_SIZE (p->mode));
	  abort ();
	  break;
	}
    }
...
void
print_multi_reg (stream, instr, base_reg, mask, hat, write_back)
     FILE * stream;
     char * instr;
     int base_reg, mask, hat, write_back;
{
  int i;
  int not_first = FALSE;

  fprintf (stream, "\t%s\t%s%s", instr, REGISTER_PREFIX, reg_names[base_reg]);
...
arm_leaf_function_p ()
{
  if (arm_leaf_function < 0)
    arm_leaf_function = leaf_function_p ();

  return arm_leaf_function;
}
     ...
     rtx first;
{
  ...
  for (insn = first; insn; insn = next_nonnote_insn (insn))
	...
	case CALL_INSN:
	  ...
	  /* There can be multiple USE statements.  */
	  while (GET_CODE (next) == INSN && GET_CODE (PATTERN (next)) == USE
     ...
     int reverse;
{
  ...
  if (frame_needed)
	...
	if (regs_ever_live[reg] && ! call_used_regs[reg])
      ...
      /* If we are really returning then restore regs pc, sp and fp
	 otherwise restore fp, sp and lr.  */
      if (! lr_save_eliminated)
	live_regs_mask |= really_return ? 0xa800 : 0x6800;
    }
  else
    {
	...
	if (regs_ever_live[reg] && ! call_used_regs[reg])
      ...
      if (live_regs_mask
	  || (regs_ever_live[LR_REGNUM] && ! lr_save_eliminated))
	live_regs_mask |= 1 << (really_return ? PC_REGNUM : LR_REGNUM);
    }
  ...
  if (live_regs_mask)
	...
	strcpy (instr,
      ...
      for (reg = 0; live_regs_mask; reg ++, live_regs_mask >>= 1)
	if (live_regs_mask & 1)
	  {
	    strcat (instr, "%|");
...
/* Obtain the last bit set.  */

static HOST_WIDE_INT
last_bit_set (value)
   HOST_WIDE_INT value;
{
  HOST_WIDE_INT shift = 31;

  while (((1 << shift) & value) == 0)
    {
      if (shift < 0)
	abort ();
      shift--;
...
/* Front end to output_add_immediate.  */

static void
output_add_imm (reg1, reg2, imm)
    int reg1, reg2, imm;
{
  rtx operands[3];

  operands[0] = gen_rtx (REG, SImode, reg1);
  operands[1] = gen_rtx (REG, SImode, reg2);
  operands[2] = GEN_INT (imm);
  output_add_immediate (operands);
}

/* Output the full function prologue, in accordance with the ARM
   Procedure Call Standard (APCS).

   There are currently 16 APCS variants, divided into four categories.
      a) 26-bit program counter (PC) or 32-bit PC.
      b) Implicit or explicit stack limit checking
      c) Pass floating point registers on the stack or in FP args
      d) Reentrant and non-reentrant

   Variants (c) and (d) are currently not implemented.

   The following example shows what the entry and exit sequences for a
   function are likely to look like
      entry:
	mov   ip, sp
	stmfd sp!, {arg_regs, work_regs, fp, ip, lr, pc}
	sub   fp, ip, #4

      exit:
	ldmea fp, {work_regs, fp, sp, pc}^

   For variadic functions, we make a contiguous argument list by using
   a slightly modified entry sequence:
	mov   ip, sp
	stmfd sp!, {a1, a2, a3, a4}
	stmfd sp!, {work_regs, fp, ip, lr, pc}
	sub   fp, ip, #4

   If we enable stack limit checking, the entry sequence for a
   non-variadic function becomes:
	mov   ip, sp
	stmfd sp!, {arg_regs, work_regs, fp, ip, lr, pc}
	sub   fp, ip, #4
	cmp   sp, sl
	bllt  |__rt_stkovf_split_small|
	sub   sp, sp, #<stack size>


   The check for stack limit violation is made at the end of the function
   prologue, by which time ip is available as a work register.  If the
   stack check fails, one of two standard run-time functions are called.
   These functions will either
	a) terminate the application
	b) extend the existing stack check (decrementing sl)
	c) allocate a new stack chunk, altering sp and sl
	   to point to it.

   For functions that are going to use less than 256 bytes of stack:
	cmp  sp, sl
	bllt |__rt_stkovf_split_small|
	sub  sp, sp, #<stack size>

      After this call, fp and sp might point to different stack chunks.

   For functions that are going to use more than 256 bytes of stack:
	sub  ip, sp, #<stack size>
	cmp  ip, sl
	bllt |__arm_stkovf_split_big|
	sub  sp, sp, #<stack size>

      After this call, fp and sp might point to different stack chunks.

   The above describes the basic layout of an APCS prologue.  However, it
   is not always necessary to conform to this exact style.  Often the
   entry sequence can be optimised according to the characteristics of the
   current function (e.g. no stack use, leaf function etc.).  */
...
output_func_prologue (f, frame_size)
     ...
     int frame_size;
{
  int reg, live_regs_mask = 0, live_regs = 0, live_float_regs = 0;
  int volatile_func = (optimize > 0
		       && TREE_THIS_VOLATILE (current_function_decl));
  int frame_needed;
  ...
  fprintf (f, "\t%s args = %d, pretend = %d, frame = %d, alloca = %d\n",
	   ASM_COMMENT_START, current_function_args_size,
	   current_function_pretend_args_size, frame_size,
	   current_function_calls_alloca);
  fprintf (f, "\t%s frame_needed = %d, anonymous_args = %d, regs_live[14] = %d\n",
	   ASM_COMMENT_START, frame_needed,
	   current_function_anonymous_args,
	   current_function_has_nonlocal_label, regs_ever_live[14]);
  fprintf (f, "\t%s nonlocal_label = %d, nonlocal_goto = %d, clobbers lr = %d\n",
	   ASM_COMMENT_START, current_function_has_nonlocal_label,
  ...
  if (frame_needed)
    {
      /* For volatile functions, we do not need to save any call used
	 registers.  */
      if (!volatile_func)
	for (reg = 0; reg <= 10; reg++)
	  if (regs_ever_live[reg] && ! call_used_regs[reg])
	    live_regs++, live_regs_mask |= (1 << reg);

      /* We can be sure that lr does not need to be saved if any of the
	 following conditions are satisfied:
	   a) not optimising
	   b) no registers to save
	   c) function does not have more than 4 arguments
	   d) function does not call alloca
	   e) function does not require any stack space
	   f) function does not clobber lr
	   g) function does not use a static chain
	   h) floating point registers don't need to be saved.  */
      if (!(optimize == 0
	    || live_regs_mask
	    || current_function_calls_alloca
	...
	lr_save_eliminated = 1;
      ...
      /* Check if any floating point registers need to be saved.  */
      if (!volatile_func)
	...
	if (current_function_needs_context)
	  {
	    if (current_function_pretend_args_size)
	      {
		/* Saving registers r0-r3 and having a static chain means
		   that we have to fixup the saved sp in the stack frame
		   later.  Set the live register mask to save fp, sp, lr
		   and pc.  */
		live_regs_mask |= 0xe800;
	      }
	    else
	      {
		/* Push registers sp, lr and pc onto the stack.  */
		print_multi_reg (f, "stmfd", STACK_POINTER_REGNUM,
				 0xe000, FALSE, 1);

		/* Set the live register mask to save fp.  */
		live_regs_mask |= 0x0800;
	      }
	  }
	else
#endif
    ...
    {
      /* We're not setting up an APCS stack frame.  */
      if (!volatile_func)
	for (reg = 0; reg <= 12; reg++)
	  if (regs_ever_live[reg] && ! call_used_regs[reg])
	    live_regs++, live_regs_mask |= 1 << reg;
      ...
      /* We can be sure that lr does not need to be saved if any of the
	 following conditions are satisfied:
	   a) no registers to save
	   b) function does not use a static chain.  */
  ...
  if (current_function_pretend_args_size)
    {
      if (store_arg_regs)
	print_multi_reg (f, "stmfd", STACK_POINTER_REGNUM,
			 ((0xf0 >> (current_function_pretend_args_size / 4))
			  & 0xf), FALSE, 1);
      else
	fprintf (f, "\tsub\t%s%s, %s%s, #%d\n",
  ...
  if (live_regs_mask)
    print_multi_reg (f, "stmfd", STACK_POINTER_REGNUM,
		     live_regs_mask, FALSE, 1);

  /* Store any call-used floating point registers.  */
  if (! volatile_func)
    {
      ...
      else
	{
	  ...
	  for (reg = 23; reg > 15; reg--)
	      ...
	      else
		{
		  if (start_reg != reg)
		    {
		      fprintf (f, "\tsfmfd\t%s%s, %d, [%s%s]!\n",
			       REGISTER_PREFIX, reg_names[reg + 1],
			       start_reg - reg, REGISTER_PREFIX,
			       reg_names[STACK_POINTER_REGNUM]);
		      live_float_regs += start_reg - reg;
		    }
		  start_reg = reg - 1;
		}
	    }
  ...
  /* This is setup for returning when we have altered sp.  */
  if (live_regs_mask && frame_needed)
#ifdef STATIC_CHAIN_INCOMING_REGNUM
    if (current_function_needs_context)
      {
	/* add fp, sp, #<number>.  */
	output_add_imm (11 /* fp */, STACK_POINTER_REGNUM,
			12 + 4 * live_regs + 12 * live_float_regs);
	fprintf (f, "\tmov\t%s%s, %s%s\n",
		 REGISTER_PREFIX, reg_names[STATIC_CHAIN_INCOMING_REGNUM],
		 REGISTER_PREFIX, reg_names[STATIC_CHAIN_REGNUM]);
      }
    else
#endif
      {
	/* sub fp, ip, #4 + (4 * stacked args).  */
	output_add_imm (11 /* fp */, 12 /* ip */,
...
#ifdef STATIC_CHAIN_INCOMING_REGNUM
  /* Fixup the value of sp saved in the stack earlier which was wrong,
     when we need to save some of a1-a4 and we have a static chain.  */
  /* XXX No guarantees for support_apcs_float.  */
  if (frame_needed && current_function_needs_context
      && current_function_pretend_args_size)
    {
      /* add ip, fp, #<stacked args>.  */
      output_add_imm (12 /* ip */, 11 /* fp */,
		      4 + current_function_pretend_args_size);
#ifdef AOF_ASSEMBLER
  if (flag_pic)
	fprintf (f, "\tstr\t%s%s, [%s%s, #-8]\n",
		 REGISTER_PREFIX, reg_names[PIC_OFFSET_TABLE_REGNUM],
		 REGISTER_PREFIX, reg_names[ARG_POINTER_REGNUM]);
      else
#endif
	fprintf (f, "\tstr\t%s%s, [%s%s, #-8]\n",
		 REGISTER_PREFIX, reg_names[12] /* ip */,
  ...
  /* Output the stack checking code if needed.  */
  if (arm_stack_check_needed () && !lr_save_eliminated)
    {
      rtx op;
      int regno = STACK_POINTER_REGNUM;

      if (frame_size > 256)
	{
	  /* Align stack check size so we can perform the subtraction
	     in just one instruction.  */
	  int align, stack_check_size;

	  /* Use ip as a scratch register for the stack limit checks.  */
	  regno = 12; /* ip */

	  /* Align size is a power of 8 less than the highest power
	     in that particular range.  */

	  align = last_bit_set (frame_size);
	  if (frame_size > (1 << align))
	    align ++;
	  align = (align + 1) & ~1;
	  align = 1 << (align - 8);
	  stack_check_size = (frame_size + 1) & ~(align - 1);
	  output_add_imm (regno, STACK_POINTER_REGNUM, -stack_check_size);
	}

      /* Different run-time environments may define their own stack
	 check support function names by re-defining ARM_STKOVF_SPLIT_BIG
	 and ARM_STKOVF_SPLIT_SMALL.  */
      fprintf (f, "\tcmp\t%s%s, %s%s\n",
	       REGISTER_PREFIX, reg_names[regno],
	       REGISTER_PREFIX, reg_names[10] /* sl */);
      op = gen_rtx (SYMBOL_REF, Pmode,
		    (frame_size > 256) ? ARM_STKOVF_SPLIT_BIG
				       : ARM_STKOVF_SPLIT_SMALL);
      assemble_external_libcall (op);
      output_asm_insn ("bllt\t%a0", &op);
   }
  if (frame_size)
    {
      /* Create the space needed for the data to be stored on the
	 stack.  */
      output_add_imm (STACK_POINTER_REGNUM,
		      STACK_POINTER_REGNUM, -frame_size);
    }

#if HARD_FRAME_POINTER_REGNUM == FRAME_POINTER_REGNUM
  /* If we are not optimising the function, or we are calling
     builtin_alloca, then make v6/r9 point to the bottom of the stack.  */
#ifdef PREVENT_FP_CLOBBER
  /* If we have defined PREVENT_FP_CLOBBER to prevent the assumed
     clobbering of the stack pointer when builtin_alloca is called,
     we do not need to make v6/r9 point to the stack.  If we are going
     to receive non-local gotos then we need to set v6 up.  */
  if (! optimize
      || (! PREVENT_FP_CLOBBER && current_function_calls_alloca)
      || current_function_has_nonlocal_label
      || current_function_has_nonlocal_goto)
    fprintf (f, "\tmov\t%s%s, %s%s\n",
	     REGISTER_PREFIX, reg_names[FRAME_POINTER_REGNUM],
	     REGISTER_PREFIX, reg_names[STACK_POINTER_REGNUM]);
#else
  if (! optimize || current_function_calls_alloca
      || current_function_has_nonlocal_label
      || current_function_has_nonlocal_goto)
    fprintf (f, "\tmov\t%R%s, %s%s\n",
	     REGISTER_PREFIX, reg_names[FRAME_POINTER_REGNUM],
	     REGISTER_PREFIX, reg_names[STACK_POINTER_REGNUM]);
#endif
#endif
     ...
     int frame_size;
{
  ...
  frame_size += current_function_outgoing_args_size; /* NAB */

  if (use_return_insn (FALSE) && return_used_this_function)
    {
      if (frame_size && ! TARGET_APCS)
	abort ();
  ...
  if (arm_apcs_frame_needed ())
      ...
      if (TARGET_THUMB_INTERWORK)
	{
	  ...
	  print_multi_reg (f, "ldmea", 11, live_regs_mask, FALSE, 0);
	  fprintf (f, "\tbx\t%slr\n", REGISTER_PREFIX);
	}
      else
	{
	  live_regs_mask |= 0xA800;
	  print_multi_reg (f, "ldmea", 11, live_regs_mask,
			   TARGET_APCS_32 ? FALSE : TRUE, 0);
	}
    }
  else
    {
	...
	if (regs_ever_live[reg] && ! call_used_regs[reg])
      ...
      if (current_function_calls_alloca || frame_size)
	{
	  operands[0] = stack_pointer_rtx;
	  if (current_function_calls_alloca)
	    operands[1] = gen_rtx_REG (SImode, FRAME_POINTER_REGNUM);
	  else
      ...
      if (current_function_pretend_args_size == 0 && regs_ever_live[LR_REGNUM])
	{
	  if (TARGET_THUMB_INTERWORK)
		...
		live_regs_mask |= 1 << LR_REGNUM;
	      ...
	      if (live_regs_mask != 0)
		print_multi_reg (f, "ldmfd", STACK_POINTER_REGNUM,
				 live_regs_mask | 0x4000, FALSE, 1);
	    ...
	    print_multi_reg (f, "ldmfd", STACK_POINTER_REGNUM,
			     live_regs_mask | 0x8000,
			     TARGET_APCS_32 ? FALSE : TRUE, 1);
	}
      else
	{
...
arm_stack_check_needed ()
{
  ...
  if (arm_leaf_function_p () && abs (get_frame_size ()) <= 256)
    return 0;
     ...
     rtx x;
{
  ...
  for (offset = 0, chainp = &aof_pic_chain; *chainp;
    ...
    if ((*chainp)->symname == XSTR (x, 0))
      {
	/* AOF assembler that doesn't support BASED areas properly.  */
     ...
     FILE * f;
{
  /* The AOF assembler needs this to cause the startup code to be extracted
     from the library.  Bringing in __main causes the whole thing to work
     automagically.  */
#if 0
