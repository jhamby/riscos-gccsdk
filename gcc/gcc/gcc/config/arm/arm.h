...
/* Register allocation in ARM Procedure Call Standard (as used on RISCiX):
   (S - saved over call).
	...
	cc		This is NOT a real register, but is used internally
			...
			codes.
	afp             This isn't either. We use it in non-apcs code
	                to allow the register allocator to use r11 as
	                an extra register.  */
...
#define HARD_REGNO_NREGS(REGNO, MODE)  					\
    (((REGNO) >= 16 && (REGNO) != ARG_POINTER_REGNUM) ? 1		\
...
#define HARD_REGNO_MODE_OK(REGNO, MODE)  			\
  ...
  ((REGNO) < 16 || (REGNO) == ARG_POINTER_REGNUM		\
...
/* Register numbers used for passing a function's static chain pointer.
   ...
   The register number as seen by the called function is r12/ip.  */
#define STATIC_CHAIN_REGNUM	12

/* The register number as seen by the calling function is r8/v5.  */
...
#define REGNO_REG_CLASS(REGNO)  			\
  (((REGNO) < 16 || (REGNO) == ARG_POINTER_REGNUM)	\
   ? GENERAL_REGS					\
   : (REGNO) == CC_REGNUM ? NO_REGS : FPU_REGS)
...
/* If defined, the maximum amount of space required for the outgoing
   arguments will be computed and put in the variable
   ...
   If defined, stack limit checking is easier and better.  Provides a
   slight performance increase, but nothing of significance, since an
   instruction was occasionally used to increment the stack pointer
...
/* Definitions for register eliminations.
   ...
   We have two registers that can be eliminated on the ARM.  The arg pointer
   register and pseudo frame pointer register can always be eliminated in
   favor of the stack pointer register.  */
...
{									\
  ...
  if ((FROM) == ARG_POINTER_REGNUM || (TO) == ARG_POINTER_REGNUM)	\
    {									\
      int regno;							\
      int offset = abs (get_frame_size ())				\
		   + current_function_outgoing_args_size;		\
      int saved_hard_reg = 0;						\
									\
      /* Registers don't need to be saved in volatile functions.  */	\
      if (! volatile_func)						\
        {								\
	...
	{								\
	  /* Take into account fp, ip, lr and pc.  */			\
	  offset += 16;							\
	}								\
      else								\
	{								\
	   /* Not targetting the APCS, only need to check r11, r12 */	\
	   /* and lr.  */						\
	  if (! volatile_func)						\
	    {								\
	      for (regno = 11; regno <= 12; regno++)			\
	        if (regs_ever_live[regno] && !call_used_regs[regno])	\
		  saved_hard_reg = 1, offset += 4;			\
	      if (regs_ever_live[LR_REGNUM] || saved_hard_reg)	       	\
	     offset += 4;						\
         }								\
	  /* Frame is now 4 too large.  */				\
	  offset -= 4;							\
	}								\
...
#define REGNO_OK_FOR_BASE_P(REGNO)				\
  ((REGNO) < 15 || (REGNO) == ARG_POINTER_REGNUM		\
...
{									\
    ...
    {									\
      /* Convert setjmp and longjmp to alternative names for extra	\
	 processing.  */						\
...
void   arm_poke_function_name STDIO_PROTO ((FILE *, char *));
int    arm_can_eliminate PROTO ((int, int));
#ifdef AOF_ASSEMBLER
Rtx    aof_pic_entry PROTO ((Rtx));
Rtx    aof_pic_entry1 PROTO ((Rtx));
