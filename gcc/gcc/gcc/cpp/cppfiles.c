     ...
     int spacer;
{
  ...
  int cr = 0;
#ifdef __riscos__
  char riscos_string[256];
  static char *riscos_dest_filename = 0;
#endif
...
#ifdef __riscos__
  /* We need to convert the string (filename) into a proper format
     understood by RISC OS to make for compatibility with Acorn's
     Make Utility.  */
  if (strcmp (string, " \\\n  ") != 0)
  ...
  size = strlen (riscos_string);
  if (size == 0)
    return;

  /* If the spacer is ':' then we are actually passing the source
     filename. We must keep a copy of this so that it can be
     duplicated for each dependency.  */
  if (spacer == ':')
    {
      if (riscos_dest_filename != 0)
        riscos_dest_filename = xrealloc (riscos_dest_filename, size + 1);
      else
        riscos_dest_filename = xmalloc (size + 1);
      strcpy (riscos_dest_filename, riscos_string);
      return;
    }
  ...
  if (spacer == ' ')
    {
      int src_size = strlen (riscos_dest_filename);

      if ((pfile->deps_size + size + src_size + 8) > pfile->deps_allocated_size)
        {
          pfile->deps_allocated_size = (pfile->deps_size + size
          			        + src_size + 50) * 2;
          pfile->deps_buffer = (char *) xrealloc (pfile->deps_buffer,
	  				     	  pfile->deps_allocated_size);
        }
      bcopy (riscos_dest_filename, &pfile->deps_buffer[pfile->deps_size], src_size);
      pfile->deps_size += src_size;
      pfile->deps_column += src_size;
      pfile->deps_buffer[pfile->deps_size++] = ':';
      pfile->deps_buffer[pfile->deps_size++] = '\t';
      bcopy (riscos_string, &pfile->deps_buffer[pfile->deps_size], size);
      pfile->deps_size += size;
      pfile->deps_buffer[pfile->deps_size++] = '\n';
      pfile->deps_column += size + 1;
    ...
    char *path;
{
    ...
    riscos_to_unix (path, tmp);
    if (strlen (path) < strlen (tmp))
      {
        /* After translation we find that the filename has grown.
           We need to resize the malloc buffer for `path'.  */
        path = (char *) xrealloc (path, strlen (tmp) + 1);
      }
    strcpy (path, tmp);
