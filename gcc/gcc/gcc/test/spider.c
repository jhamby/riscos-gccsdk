/* You may not remember that I e-mailed you about gcc compiling perl with a
BL __gccmain
which caused it to crash upon running.
perl is purely C, not C++, and removing this line from the assembler output
makes it work fine. */

/*I've found a smaller piece of code which exhibits the same problem:
------------------------------------------------------------------------------- */
/*
 * win95ping.c
 *
...
When compiled with gcc and developer UnixLib this too attempts to branch to
__gccmain, with disasterous results.
Removing this branch fixed the problem
[However, it still won't crash the test-case PC next door.
Unlike
use IO::Socket;
IO::Socket::INET
	->new(PeerAddr=>"pc004232.esc.liv.ac.uk:139",Proto=>'tcp')
	->send("Die sucker", MSG_OOB)

[yes, 1: This does work on my perl port
      2: You can crash that specific machine. Please don't
      3: I really ought to find an install the patch from M$
      4: I'm not crashing other peoples machines - I'm just curious to see
         what works
]
*/
