/* RISC OS to Unix format file name conversion.
   Copyright (c) 1996, 1997, 1998, 1999, 2000, Nick Burrett.
...
{
  "def", "f", "for", "fpp", "p", "pas", "ph", "gpi",
  "cc", "cxx", "cpp", "c++", "c", "m", "rpo",
  "i", "ii", "h", "i", "s", "l", "o", "y", "java",
...
/* Return 1 if `name' contains exactly one dot.  */
static int
...
main (void)
{
  ...
  int flags = __riscosify_control;/* __RISCOSIFY_DONT_TRUNCATE; */
  ...
  riscos_to_unix ("machmode.def", out);
  __riscosify (out, 0, flags, name, sizeof (name), NULL);
  printf ("out = '%s', name = '%s'\n", out, name);
  return 0;
}
