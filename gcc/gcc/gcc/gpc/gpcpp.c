/* C Compatible Compiler Preprocessor (CCCP)
   ...
   Modified to handle mixed, nested and Delphi comments, Frank Heckenbach, Mar 2000
...
static void check_isalnum PROTO((void));
static void
check_isalnum ()
{
  /* Check for a known bug in some libc distributions.
     This must be done before redefining isalnum() etc., to be effective. */
  if (! isalnum ('0'))
    {
      warning ("nonfunctional `isalnum()' detected");
      warning (" (In some distributions of the old Linux libc5 the `isalnum()'");
      warning (" function does not work, causing programs using that library");
      warning (" - like this build of GPC - to fail.  We suggest that you fix");
      warning (" your libc5 by upgrading to a more recent version like 5.4.46.");
      warning (" You can also upgrade to a GPC that works with GNU libc2 (libc6).)");
    }
}

/* Don't use the locale dependent routines. */
static int new_isdigit PROTO ((int));
static int new_isprint PROTO ((int));
...
/* Nonzero means handle C++/Delphi style comments */
...
/* Nonzero means that a Borlandish Pascal dialect is used */

static int borland_pascal_dialect = 0;

/* Character to start directives. Used in error messages. */
...
/* Nonzero means don't expand any macros */
...
static int mixed_comments_notified = 0;

/* Nozero means comments may be nested */

static int nested_comments = 0;
static int warn_nested_comments = 0;
static int nested_comments_notified = 0;
  ...
  if (indepth >= (INPUT_STACK_MAX - 1))                                 \
    {                                                                   \
      error_with_line (line_for_error (instack[indepth].lineno),        \
                       "macro or `%cinclude' recursion too deep", directive_char); \
      code;                                                             \
    }
...
#ifdef GPC
 T_IFOPT,       /* the Pascal `{$ifopt}' directive */
#endif
...
/* FH: Made the hash function case-insensitive to allow for
   case-insensitive defines in Pascal. */
...
static struct option_list *current_options = NULL_PTR;
...
/* Structure used in Pascal to keep track of local options */
...
static struct local_option_list *local_options_stack = NULL_PTR;
...
static int do_define1 PROTO((U_CHAR *, U_CHAR *, FILE_BUF *, struct directive *, int, HASHNODE **));
...
/* table to tell if char can be part of an identifier. */
U_CHAR is_idchar[256];
/* table to tell if char can be first char of an identifier. */
...
static void warn_mixed_comment PROTO((void));
static void warn_nested_comment PROTO((void));

static U_CHAR *skip_one_comment PROTO((U_CHAR *, U_CHAR *, int *, int));

static void rescan PROTO((FILE_BUF *, int));

static FILE_BUF expand_to_temp_buffer PROTO((U_CHAR *, U_CHAR *, int, int));

static char *check_negative_option PROTO((char *, int, int, int *));

static int handle_option PROTO((char *));
...
static HASHNODE *install PROTO((U_CHAR *, int, enum node_type, char *, int, int));
...
static void make_definition PROTO((char *, FILE_BUF *, int));
...

/* Check if a `/'`*', `(*' or `//' comment (when appropriate) starts at p */
#define IS_COMMENT_START(p) ((*(p) == comment_start_char && (p)[1] == '*') \
                             || (cplusplus_comments && *(p) == '/' && (p)[1] == '/'))

/* Check if a `/'`*', `(*' or `{' comment (when appropriate) starts at p */
#define IS_COMMENT_START2(p) ((*(p) == comment_start_char && (p)[1] == '*') \
                              || (pascal_comments && *(p) == '{'))

...
/* Stores into *is_negative whether option is negative. Returns a
   newly allocated string. If make_positive is zero, it contains
   option negated, otherwise it contains the positive version of
   option, whether or not options was positive or negative. */
static char *
check_negative_option (option, length, make_positive, is_negative)
     char *option;
     int length;
     int make_positive;
     int *is_negative;
{
  char *result;
  #define NEGATION "no-"
  *is_negative = length >= 2 + strlen (NEGATION)
     ...
     char *option;
{
  ...
  char *p, *name = option + 2, *positive_option, *temp;
  int j, is_negative, length;
  struct option_list *po, **ppo;
  while (*map->src)
    {
      char **src = map->src;
      while (*src && strcmp (option, *src) != 0)
        src++;
      if (*src)
        {
          char **dest = map->dest;
  ...
  if (is_negative)
    {
      ppo = &current_options;
      while (*ppo && strcmp ((*ppo)->option, positive_option) != 0)
        ppo = &((*ppo)->next);
      if (*ppo)
        {
          po = *ppo;
    ...
    switch (option [1])
      {
        case 'f':
          if (!strcmp (name, "macros"))
            ...
            {
              if (!lang_pascal)
                warning ("Must give `-lang-pascal' option to allow mixed Pascal comments");
              else
                mixed_comments = 1;
            }
            ...
            {
              if (!lang_pascal)
                warning ("Must give `-lang-pascal' option to allow nested Pascal comments");
              else
                nested_comments = 1;
            }
            ...
            {
              if (!lang_pascal)
                warning ("Must give `-lang-pascal' option to allow Delphi comments");
              else
            ...
            cplusplus_comments = 0;
                   ...
                   !strcmp (name, "delphi"))
            borland_pascal_dialect = 1;
          else if (!strcmp (name, "gnu-pascal"))
            borland_pascal_dialect = 0;
        ...
        case 'W':
          if (!strcmp (name, "warnings"))
            ...
            inhibit_warnings = 1;
          else if (!strcmp (name, "trigraphs"))
            warn_trigraphs = 1;
          else if (!strcmp (name, "no-trigraphs"))
            warn_trigraphs = 0;
          else if (!strcmp (name, "comment"))
            warn_comments = 1;
          else if (!strcmp (name, "no-comment"))
            warn_comments = 0;
          else if (!strcmp (name, "comments"))
            warn_comments = 1;
          else if (!strcmp (name, "no-comments"))
            warn_comments = 0;
          else if (!strcmp (name, "traditional"))
            warn_stringify = 1;
          else if (!strcmp (name, "no-traditional"))
            warn_stringify = 0;
          else if (!strcmp (name, "import"))
            warn_import = 1;
          else if (!strcmp (name, "no-import"))
            warn_import = 0;
  ...
  j = 0;
  while ((temp = gpc_options[j].name) != NULL_PTR &&
         (strlen (temp) != length ||
          strncmp (temp, option, length) != 0))
    j++;
     ...
     char **argv;
{
  ...
  bzero ((char *) pend_defs_case_sensitive, argc * sizeof (int));
  ...
  for (i = 1; i < argc; i++) {
	...
	if (!strcmp (argv[i], "-icrossdirafter")) {
          struct file_name_list *dirtmp;
          char *prefix;
          ...
          if (include_prefix != 0)
            ...
            prefix = savestring (CROSS_INCLUDE_DIR);
          dirtmp = (struct file_name_list *)
            xmalloc (sizeof (struct file_name_list));
          dirtmp->next = 0;     /* New one goes on the end */
          dirtmp->control_macro = 0;
          dirtmp->c_system_include_path = 0;
          if (i + 1 == argc)
            fatal ("Directory name missing after `-icrossdirafter' option");
      ...
      case 'l':
        ...
        if (! strcmp (argv[i], "-lang-objc"))
          objc = 1, cplusplus = 0, cplusplus_comments = 1, c89 = 0;
        if (! strcmp (argv[i], "-lang-objc++"))
          objc = 1, cplusplus = 1, cplusplus_comments = 1, c89 = 0;
        if (! strcmp (argv[i], "-lang-pascal")) {
          char **option;

          cplusplus = 0, cplusplus_comments = 0, c89 = 0, objc = 0;
          ...
          /* Process default options. We must not do this before the
             loop, (a) in case the preprocessor is ever user for
             anything else than Pascal, and (b) because some of the
             options are only accepted after `-lang-pascal'. */
      ...
      case 'v':
#ifdef GPC
        fprintf (stderr, "GNU Pascal Compiler PreProcessor version %s", version_string);
#else
  ...
  for (i = 1; i < argc; i++) {
    ...
    if (pend_defs[i]) {
      if (debug_output)
        output_line_directive (fp, &outbuf, 0, same_file);
      make_definition (pend_defs[i], &outbuf, pend_defs_case_sensitive[i]);
    }
  ...
  if (print_deps) {
    ...
    if (deps_target) {
      ...
      q = rindex (in_fname, '/');
#ifdef DIR_SEPARATOR
      if (q == NULL) q = rindex (in_fname, DIR_SEPARATOR);
      else {
        char *tmp = rindex (in_fname, DIR_SEPARATOR);
        if (tmp != NULL && tmp > q) q = tmp;
      }
#endif
      if (q)
        ++q;
     ...
     FILE_BUF *buf;
{
  ...
  if (warn_trigraphs && fptr != bptr)
    warning_with_line (0, "%d trigraph(s) encountered", (int) (fptr - bptr) / 2);
}

...
warn_mixed_comment ()
{
  if (warn_mixed_comments && !mixed_comments_notified && !inhibit_warnings)
    {
      mixed_comments_notified = 1;
...
warn_nested_comment ()
{
  if (warn_nested_comments && !nested_comments_notified && !inhibit_warnings)
    {
      nested_comments_notified = 1;
      warning ("nested comments are a GPC extension");
    }
}

/* Skip the (non-C++) comment starting at bp, handling C, Pascal,
     ...
     int fix_newlines;
{
  ...
  if (*bp == '{')
    ...
    {
      bp += 2;
      comment1 = pascal_comments && mixed_comments;
    }
  while (*bp && comment_count != 0 && (!limit || bp < limit))
    {
      ...
      if (fix_newlines && bp1 == '\\' && (!limit || bp + 2 < limit) && bp[2] == '\n')
        newline_fix (bp + 1);
      if (comment2 && *bp == '*' && bp1 == comment_end_char)
        {
      ...
      else if (nested_comments && comment2 && *bp == comment_start_char && bp1 == '*')
        {
          if (comment2 == 1) warn_mixed_comment ();
          warn_nested_comment ();
          comment_count++;
      ...
      else if (nested_comments && comment1 && *bp == '{')
        {
          if (comment1 == 1) warn_mixed_comment ();
          warn_nested_comment ();
          comment_count++;
...
do { ip = &instack[indepth];            \
  ...
  while (1) {
    ...
    *obp++ = c;
...
#ifdef GPC
            /* might be a Borland style character constant (`#42' or `#$f0') */
            && (!lang_pascal || ((*ibp < '0' || *ibp > '9') && *ibp != '$'))
#endif
          )
    ...
    case '\'':
      /* A single quoted string is treated like a double (unless in Pascal) -- some
      ...
      while (1) {
        ...
        *obp++ = *ibp;
        ...
        case '\\':
          if (no_charescape || (lang_pascal && c == '\''))
            break;
          if (ibp >= limit)
            break;
          if (*ibp == '\n') {
        ...
        while (ibp < limit) {
          ...
          case '(':
            if (!no_charescape && *ibp == '\\' && ibp[1] == '\n')
              newline_fix (ibp);
            if (ibp < limit && *ibp == '*' && nested_comments && mixed_comments)
              {
                warn_mixed_comment ();
                warn_nested_comment ();
                ibp++;
    ...
    case '(':
...

      {
        ...
        while (ibp < limit) {
          switch (*ibp++) {
          case '{':
            if (nested_comments && pascal_comments && mixed_comments)
              {
                warn_mixed_comment ();
                warn_nested_comment ();
                comment_count++;
            ...
            break;
          case '}':
            if (pascal_comments && mixed_comments)
              {
                warn_mixed_comment ();
                if (--comment_count == 0)
                  goto comment_end;
          ...
          case '(':
            if (!no_charescape && *ibp == '\\' && ibp[1] == '\n')
              newline_fix (ibp);
            if (nested_comments && pascal_comments && warn_comments && *ibp == '*')
              {
                warn_nested_comment ();
                ibp++;
                comment_count++;
          ...
          case '*':
            if (!no_charescape && *ibp == '\\' && ibp[1] == '\n')
              newline_fix (ibp);
            if (ibp >= limit || (*ibp == comment_end_char && --comment_count == 0))
              {
                ibp++;
                goto comment_end;
              }
...
randomchar:
      ...
      if (ident_length > 0) {
        ...
        if (!(pcp_outfile || no_macros) || pcp_inside_if) {
                  ...
                  while (1) {
                    ...
                    /* A comment: copy it unchanged or discard it.  */
                      ...
                      while (ibp + 1 != limit && comment_count != 0) {
                        int l = 1;
                        if (comment2 && ibp[0] == '*' && ibp[1] == comment_end_char)
                          {
                            if (comment2 == 1) warn_mixed_comment ();
                            comment_count--;
                            l = 2;
                        ...
                        else if (nested_comments && comment2 && *ibp == comment_start_char && ibp[1] == '*')
                          {
                            if (comment2 == 1) warn_mixed_comment ();
                            warn_nested_comment ();
                            comment_count++;
                        ...
                        else if (nested_comments && comment1 && *ibp == '{')
                          {
                            if (comment1 == 1) warn_mixed_comment ();
                            warn_nested_comment ();
                            comment_count++;
  ...
  if (if_stack != ip->if_stack)
    {
      ...
      error_with_line (line_for_error (if_stack->lineno),
                       "unterminated `%c%s' conditional", directive_char, str);
  }
...
#ifdef GPC
    /* Don't confuse Borland-style #13 character constants with line numbers.
     * (See gpc-lex.c for details.)
     */
    if (p != ident && this_can_be_a_char_constant)
      {
        *op->bufp = '#';
        op->bufp++;
 ...
 *   (*$M Hello! *)     --   output message "Hello!" to stderr (the compiler does this)
 *   (*$W-*)            --   inhibit warnings
 */
...
handle_pascal_directive (ip, op, limit)
    ...
    U_CHAR *limit;
{
  U_CHAR *p = ip->bufp, *q, *r, save_char, *option;
  int count = 0, option_allocated;
  struct directive *kt;
  ...
  while (p < limit)
    {
      ...
      if (nested_comments && IS_COMMENT_START2 (p))
        {
          warn_nested_comment ();
          q = skip_one_comment (p, limit, NULL, !no_charescape);
          while (p < q)
            *p++ = ' ';
        }
      else if (*p == '\t' || *p == '\n' || *p == '\r')
  ...
  while (ip->bufp < limit)
    {
      ...
      for (q = p; q < limit && *q != ' ' && *q != ','; q++)
        if (*q >= 'A' && *q <= 'Z')
          *q += 'a' - 'A';
      if (count == 0 && !strncmp (p, D_ENDLOCAL, strlen (D_ENDLOCAL)))
        {
          for (; q < limit && *q == ' '; q++);
          if (q < limit)
              ...
              while (options)
                {
                  ...
                  if (option [0] == '-' && option [1] == 'W')
                    ...
                    option += 2;
                  length = strlen (option);
                  check_expand (op, op->length - (op->bufp - op->buf) + length + 4);
                  *op->bufp++ = '{';
                  *op->bufp++ = '$';
                  bcopy (option, op->bufp, length);
                  op->bufp += length;
                  *op->bufp++ = '}';
                  free (options->option);
                  temp_o = options;
      ...
      if (count++ == 0 && !strncmp (p, D_LOCAL, strlen (D_LOCAL)))
        {
          local_options = (struct local_option_list *) xmalloc (sizeof (struct local_option_list));
          local_options->options = NULL_PTR;
          local_options->hashnodes = NULL_PTR;
      ...
      if (r - p == 2 && (p [1] == '+' || p [1] == '-'))
        {
          ...
          while (short_option->short_name
                 && (short_option->short_name != p [0]
                     || (!short_option->bp_option && borland_pascal_dialect)))
            short_option++;
      ...
      else
        {
          *r = 0;
          if (p [0] == 'w' && p [1] == ' ')
            ...
            {
              option = (U_CHAR *) xmalloc (strlen (p) + 3);
              option [0] = '-';
              option [1] = 'f';
              strcpy (option + 2, p);
              option_allocated = 1;
      ...
      if (option)
        {
          int option_handled, add_local_option = 0;
          if (local_options)
            {
              int is_negative;
              ...
              while (po && strcmp (po->option, positive_option) != 0)
                po = po->next;
              add_local_option = (po == NULL_PTR) != is_negative; /* `!=' means xor */
              free (positive_option);
            }
          option_handled = handle_option (option);
          if (option_handled && add_local_option)
            {
              int is_negative;
              struct option_list *temp = (struct option_list *) xmalloc (sizeof (struct option_list));
              temp->option = check_negative_option (option, strlen (option), 0, &is_negative);
      ...
      if (!strncmp (p, D_CSDEFINE, strlen (D_CSDEFINE)))
        {
          define_start = p + strlen (D_CSDEFINE);
          case_sensitive = 1;
        }
      if (!strncmp (p, D_CIDEFINE, strlen (D_CIDEFINE)))
        define_start = p + strlen (D_CIDEFINE);
      if (!strncmp (p, D_DEFINE, strlen (D_DEFINE)))
        define_start = p + strlen (D_DEFINE);
      if (define_start)
        {
          HASHNODE *hashnode = NULL_PTR;
      ...
      if (ip->bufp > limit)
        {
          /* This happens when processing an $if...
             Skip back to the beginning of the directive, but not
             when at the end of the buffer, otherwise error messages
             about unterminated `if's can get lost because a
             previous `endif' is read again, and the line count may
             become wrong (fjf427*.pas). */
          ip->lineno--;
          q = ip->bufp;
     ...
     FILE_BUF *op;
{
  ...
  if (pcp_outfile && pcp_inside_if
    ...
    error ("predefined macro `%s' used inside `%cif' during precompilation",
           hp->name, directive_char);
...
get_filename:
  ...
  fail:
    if (retried) {
      error ("`%c%s' expects \"FILENAME\" or <FILENAME>", directive_char, keyword->name);
      return 0;
    } else {
  ...
  if (flen == 0)
    {
      error ("empty file name in `%c%s'", directive_char, keyword->name);
      return 0;
    }
...
#ifdef DIR_SEPARATOR
         && fname[strlen (fname) - 1] != DIR_SEPARATOR
#endif
        )
          strcat (fname, "/");
      } else {
...
#ifdef DIR_SEPARATOR
         && dirname[dirlen - 1] != DIR_SEPARATOR
#endif
          ;
     ...
     struct file_name_list *searchptr;
{
  ...
  p = rindex (fname, '/');
#ifdef DIR_SEPARATOR
  if (! p) p = rindex (fname, DIR_SEPARATOR);
  else {
    char *tmp = rindex (fname, DIR_SEPARATOR);
  ...
  if (searchptr
      && searchptr->fname
      && strlen (searchptr->fname) == p - fname
      && ! strncmp (searchptr->fname, fname, p - fname))
    {
      /* FILENAME is in SEARCHPTR, which we've already checked.  */
      return open (fname, O_RDONLY, 0666);
    }
  ...
  if (p == fname)
    ...
    {
      dir = (char *) alloca (p - fname + 1);
      bcopy (fname, dir, p - fname);
      dir[p - fname] = '\0';
      from = p + 1;
    }
     ...
     struct file_name_list *dirptr;
{
  ...
  if (S_ISREG (st_mode)) {
    ...
    error ("directory `%s' specified in %cinclude", fname, directive_char);
    close (f);
    return;
  } else {
    ...
    *stringlist_tailp = str;
    ...
    if (nkeys == -1)
      ...
      for (; nkeys--; free (tmpbuf.buf), cp = endofthiskey + 1) {
        ...
        if (hp == NULL) {
          kp->chain = 0;
          install (tmpbuf.bufp, -1, T_PCSTRING, (char *) kp, -1, 1);
        }
  ...
  if (*bp == '(') {
    ...
    while (*bp != ')') {
      ...
      if (temp->length == 0 || (*bp != ',' && *bp != ')')) {
        error ("badly punctuated parameter list in `%cdefine'", directive_char);
        goto nope;
      }
      if (*bp == ',') {
        bp++;
      ...
      if (bp >= limit) {
        error ("unterminated parameter list in `%cdefine'", directive_char);
        goto nope;
      }
      {
        ...
        for (otemp = temp->next; otemp != NULL; otemp = otemp->next)
          if (temp->length == otemp->length &&
              ...
              error ("duplicate argument name `%.*s' in `%cdefine'",
                     temp->length, temp->name, directive_char);
              goto nope;
          }
      }
    }
    ...
    if (bp < limit)
      {
        if (is_hor_space[*bp]) {
          ...
          switch (*bp) {
            ...
            case '?':  case '[':  case '\\': case ']':  case '^':  case '{':
            case '|':  case '}':  case '~':
              warning ("missing white space after `%cdefine %.*s'",
                       directive_char, sym_length, symname);
              break;

            default:
              pedwarn ("missing white space after `%cdefine %.*s'",
                       directive_char, sym_length, symname);
              break;
          }
        }
  ...
  while (p < limit) {
    ...
    *exp_p++ = c;
    ...
    if (!traditional) {
      ...
      case '\\':
        if (!no_charescape && p < limit && expected_delimiter && (!lang_pascal || expected_delimiter != '\'')) {
     ...
     struct directive *keyword;
{
  ...
  if (pedantic && done_initializing && !instack[indepth].system_header_p)
    pedwarn ("%s does not allow `%cassert'", language, directive_char);
  ...
  if (*bp != '(') {
    error ("missing token-sequence in `%cassert'", directive_char);
    return 1;
  }

  {
    ...
    if (error_flag)
      return 1;
    if (tokens == 0) {
      error ("empty token-sequence in `%cassert'", directive_char);
      return 1;
    }
     ...
     struct directive *keyword;
{
  ...
  if (pedantic && done_initializing && !instack[indepth].system_header_p)
    pedwarn ("%s does not allow `%cunassert'", language, directive_char);
  ...
  if (*bp == '(') {
    ...
    if (error_flag)
      return 1;
    if (tokens == 0) {
      error ("empty token list in `%cunassert'", directive_char);
      return 1;
    }
     ...
     int *error_flag;
{
  ...
  *error_flag = 0;
  ...
  while (depth > 0) {
    ...
    if (bp >= limit) {
      error ("unterminated token sequence in `%cassert' or `%cunassert'",
     ...
     struct directive *keyword;
{
  ...
  if (pedantic) {
    ...
    SKIP_WHITE_SPACE (buf);
    if (buf != limit)
      pedwarn ("garbage after `%cundef' directive", directive_char);
  }
  return 0;
}

     ...
     struct directive *keyword;
{
  ...
  if (pedantic && !instack[indepth].system_header_p)
    pedwarn ("%s does not allow `%cident'", language, directive_char);
...
#ifdef DIR_SEPARATOR
      if (inc_fname == NULL) inc_fname = (U_CHAR *) rindex (ptr->fname, DIR_SEPARATOR);
      else {
        U_CHAR *tmp = (U_CHAR *) rindex (ptr->fname, DIR_SEPARATOR);
        if (tmp != NULL && tmp > inc_fname) inc_fname = tmp;
      }
#endif
     ...
     struct directive *keyword;
{
  ...
  if (if_stack == instack[indepth].if_stack) {
    error ("`%celif' not within a conditional", directive_char);
    return 0;
  } else {
    if (if_stack->type != T_IF && if_stack->type != T_ELIF && if_stack->type != T_IFOPT) {
      error ("`%celif' after `%celse'", directive_char, directive_char);
...
{
  /*
    @@This is a quick hack to make the preprocessor accept
    Pascal-like syntax in `if' expressions. This should be good
    enough for most purposes. If more similarity to Pascal is
    wanted, one might have to rewrite cexp.y for Pascal. Some
    C fanatics might think that the following table is all it takes
    to turn a C compiler into a Pascal compiler, but that's not
    quite so... ;-)
  */
  const struct pascal_operator
  {
    U_CHAR *pascal_name, *c_name;
  } pascal_operators[] =
    {
      { "not", "!" },
      { "and", "&&" },
      { "or", "||" },
      { "xor", "!=" }, /* for boolean values, it's the same... */
      { "shl", "<<" },
      { "shr", ">>" },
      { "false", "0" },
      { "true", "1" }
    };
  #define NUMBER_PASCAL_OPERATORS (sizeof (pascal_operators) / sizeof (struct pascal_operator))
  FILE_BUF temp_obuf;
  HASHNODE *save_defined, *save_pascal_operators [NUMBER_PASCAL_OPERATORS];
  HOST_WIDE_INT value;
  int i;

  save_defined = install ((U_CHAR *) "defined", -1, T_SPEC_DEFINED,
                          NULL_PTR, -1, !lang_pascal);
  if (lang_pascal)
    for (i = 0; i < NUMBER_PASCAL_OPERATORS; i++)
      {
        U_CHAR *pascal_name = pascal_operators [i].pascal_name;
        DEFINITION *defn = (DEFINITION *) xcalloc (1, sizeof (DEFINITION));
        defn->length = strlen ((defn->expansion = pascal_operators [i].c_name));
        defn->nargs = -1;
        defn->args.argnames = (U_CHAR *) "";
        defn->line = 0;
        defn->file = NULL_PTR;
        defn->predefined = 0;
        save_pascal_operators [i] = install (pascal_name,
          strlen (pascal_name), T_MACRO, (char *) defn, -1, 0);
      }

  /* This also makes sure `defined' and the Pascal operators are
     evaluated even if no_macros is set. */
  pcp_inside_if = 1;
  ...
  if (lang_pascal)
    {
      char *temp_buf, *p, *q, *limit;
      for (i = 0; i < NUMBER_PASCAL_OPERATORS; i++)
        delete_macro (save_pascal_operators [i]);
      q = temp_buf = (char *) xmalloc (2 * temp_obuf.length + 2); /* worst case */
      p = (char *) temp_obuf.buf;
      limit = p + temp_obuf.length;
      while (p < limit)
        if (*p == '<' && p[1] == '>')
          {
            *q++ = '!';
      ...
      *q++ = '\n';
      *q = 0; /* might not be necessary */
      value = parse_c_expression (temp_buf);
      free (temp_buf);
    }
  else
    {
     ...
     struct directive *keyword;
{
  ...
  if (end == buf) {
    ...
    if (! traditional)
      pedwarn (end == limit ? "`%c%s' with no argument"
               : "`%c%s' argument starts with punctuation",
               directive_char, keyword->name);
  } else {
    HASHNODE *hp;

    if (pedantic && buf[0] >= '0' && buf[0] <= '9')
      pedwarn ("`%c%s' argument starts with a digit",
        directive_char, keyword->name);
    else if (end != limit && !traditional)
      pedwarn ("garbage at end of `%c%s' argument",
        directive_char, keyword->name);
...
{
  FILE_BUF *ip = &instack[indepth];
  int skip = 1, is_negative, length, this_length = 0, option_ok = 0;
  char *copy, *copy_option, *this_option = NULL_PTR, *p;
  ...
  copy_option = copy + 2;
  strncpy (copy_option, buf, length);
  ...
  if (length == 2 && (copy_option [1] == '+' || copy_option [1] == '-'))
    {
      const struct gpc_short_option *short_option = gpc_short_options;
      while (short_option->short_name
             && (short_option->short_name != copy_option [0]
                 || (!short_option->bp_option && borland_pascal_dialect)))
        short_option++;
      if (short_option->short_name)
        {
  ...
  if (!option_ok && this_option)
    {
      ...
      while ((option = gpc_options[j].name) != NULL_PTR &&
             (strlen (option) != this_length ||
              strncmp (option, this_option, this_length) != 0))
        j++;
  ...
  if (option_ok)
    {
      ...
      while (po && strcmp (po->option, positive_option) != 0)
        po = po->next;
      skip = (po == NULL_PTR) != is_negative; /* `!=' means xor */
      free (positive_option);
    }
  else
    error ("unknown option `%.*s' in `%cifopt'\n", length, buf, directive_char);
  free (copy);
  conditional_skip (ip, skip, T_IFOPT, 0, op);
  return 0;
}
     ...
     FILE_BUF *op;
{
  ...
  while (bp < endb) {
    ...
    case '(':
      ...
      break;
#ifndef GPC  /* Strings are nothing special in a non-active conditional */
    case '\"':
    case '\'':
          ...
          case T_ELIF:
            ...
            if (kt->type != T_ENDIF) {
              if (if_stack->type == T_ELSE)
                error ("`%celse' or `%celif' after `%celse'",
                  directive_char, directive_char, directive_char);
     ...
     struct directive *keyword;
{
  ...
  if (if_stack == instack[indepth].if_stack) {
    error ("`%celse' not within a conditional", directive_char);
    return 0;
  } else {
    ...
    if (if_stack->type != T_IF && if_stack->type != T_ELIF && if_stack->type != T_IFOPT) {
      error ("`%celse' after `%celse'", directive_char, directive_char);
      fprintf (stderr, " (matches line %d", if_stack->lineno);
     ...
     struct directive *keyword;
{
  ...
  if (if_stack == instack[indepth].if_stack)
    error ("unbalanced `%cendif'", directive_char);
  else {
     ...
     register U_CHAR *p;
{
  ...
  while (1) {
    ...
    if (is_hor_space[*p])
      p++;
    else if (*p == comment_start_char || *p == '/' || *p == '{') {
      ...
      if (IS_COMMENT_START2 (p)) {
        ...
        p = skip_one_comment (p, NULL, NULL, !no_charescape);
      }
      else if (cplusplus_comments && *p == '/' && p[1] == '/') {
        p += 2;
        while (*p && (*p != '\n' || (p[-1] == '\\' && !no_charescape)))
          p++;
      }
  ...
  if (*p && *p != '\n')
    {
      if (lang_pascal)
        /* In Pascal, there doesn't have to be a newline after a
           directive.
        pedwarn ("ignoring text following `$else' or `$endif'")*/;
      else
     ...
     int match;
{
  ...
  if (cplusplus_comments && bp[-2] == '/' && bp[-1] == '/') {
    if (output) {
      while (bp < limit) {
        *op->bufp++ = *bp;
        if (*bp == '\n' && (bp[-1] != '\\' || no_charescape))
  ...
  while (bp < limit) {
    ...
    switch (*bp++) {
    case '{':
      if (pascal_comments && nested_comments && (match == '}' || mixed_comments))
        {
     ...
     int *eofp;
{
  ...
  while (1) {
    ...
    c = *bp++;
    if (!no_charescape && c == '\\' && (!lang_pascal || match != '\'')) {
      while (*bp == '\\' && bp[1] == '\n') {
     ...
     enum file_change_code file_change;
{
  ...
  /* Tell cc1plus or gpc1 if following text should be treated as C.  */
  if (ip->system_header_p == 2 && (cplusplus || lang_pascal)) {
#else
  if (ip->system_header_p == 2 && lang_pascal) {
#endif
    *line_end++ = ' ';
    *line_end++ = '4';
  }
        ...
        if (ap->stringify != 0) {
          ...
          for (; i < arglen; i++) {
            ...
            if (escaped)
              ...
              if (!no_charescape && c == '\\' && (!lang_pascal || in_string != '\''))
                escaped = 1;
              if (in_string) {
                if (c == in_string)
                  in_string = 0;
  ...
  while (bp < limit) {
    ...
    case '{':
      if (! pascal_comments)
        break;
      *comments = 1;
      bp = skip_one_comment (bp, limit, newlines, !no_charescape);
    ...
    case '\"':
      {
        ...
        for (quotec = *bp++; bp + 1 < limit && *bp != quotec; bp++) {
          if (!no_charescape && *bp == '\\' && (!lang_pascal || quotec != '\'')) {
            bp++;
            if (*bp == '\n')
              ++*newlines;
            while (*bp == '\\' && bp[1] == '\n') {
  ...
  while (ibp < limit) {
    *obp++ = c = *ibp++;
    ...
    case '{':
      ...
      ibp = skip_one_comment (ibp - 1, limit, NULL, !no_charescape);
      /* Comments are equivalent to spaces.  */
    ...
    case '(':
      ...
      if (c != comment_start_char || ibp[0] != '*' || ibp + 1 >= limit)
        break;
      ibp = skip_one_comment (ibp - 1, limit, NULL, !no_charescape);
      /* Comments are equivalent to spaces.
         For -traditional, a comment is equivalent to nothing.  */
         ...
         and so we don't duplicate newlines in them.  */
      {
        ...
        while (ibp < limit) {
          *obp++ = c = *ibp++;
          ...
          if (c == '\n' && quotec == '\'')
            break;
          if (!no_charescape && c == '\\' && (!lang_pascal || quotec != '\'') && ibp < limit) {
            while (*ibp == '\\' && ibp[1] == '\n')
              ibp += 2;
            *obp++ = *ibp++;
          }
...
initialize_builtins (inp, outp)
     ...
     FILE_BUF *outp;
{
  install ((U_CHAR *) "__LINE__", -1, T_SPECLINE, NULL_PTR, -1, 1);
  install ((U_CHAR *) "__DATE__", -1, T_DATE, NULL_PTR, -1, 1);
  install ((U_CHAR *) "__FILE__", -1, T_FILE, NULL_PTR, -1, 1);
  install ((U_CHAR *) "__BASE_FILE__", -1, T_BASE_FILE, NULL_PTR, -1, 1);
  install ((U_CHAR *) "__INCLUDE_LEVEL__", -1, T_INCLUDE_LEVEL, NULL_PTR, -1, 1);
  install ((U_CHAR *) "__VERSION__", -1, T_VERSION, NULL_PTR, -1, 1);
#ifndef NO_BUILTIN_SIZE_TYPE
  install ((U_CHAR *) "__SIZE_TYPE__", -1, T_SIZE_TYPE, NULL_PTR, -1, 1);
#endif
#ifndef NO_BUILTIN_PTRDIFF_TYPE
  install ((U_CHAR *) "__PTRDIFF_TYPE__ ", -1, T_PTRDIFF_TYPE, NULL_PTR, -1, 1);
#endif
  install ((U_CHAR *) "__WCHAR_TYPE__", -1, T_WCHAR_TYPE, NULL_PTR, -1, 1);
  ...
  install ((U_CHAR *) "__IMMEDIATE_PREFIX__", -1, T_IMMEDIATE_PREFIX_TYPE,
           NULL_PTR, -1, 1);
  install ((U_CHAR *) "__TIME__", -1, T_TIME, NULL_PTR, -1, 1);
  if (!traditional) {
    install ((U_CHAR *) "__STDC__", -1, T_CONST, "1", -1, 1);
    install ((U_CHAR *) "__STDC_VERSION__", -1, T_CONST, "199409L", -1, 1);
  }
  if (objc)
    install ((U_CHAR *) "__OBJC__", -1, T_CONST, "1", -1, 1);
/*  This is supplied using a -D by the compiler driver
    so that it is present only when truly compiling with GNU C.  */
/*  install ((U_CHAR *) "__GNUC__", -1, T_CONST, "2", -1, 1);  */
...
static void
make_definition (str, op, case_sensitive)
     char *str;
     FILE_BUF *op;
     int case_sensitive;
{
  ...
  do_define1 (buf, buf + strlen ((char *) buf), NULL_PTR, kt, case_sensitive, NULL);
  --indepth;
}
