...
#ifdef GPC
/* Name of the automake temp file, chosen once per project.
 */
static char *automake_temp_file_name = NULL;

/* This will be 1 if an `-o' switch has been given.
 */
static int explicit_output_file = 0;
...
/* This will be 1 if the driver program was called other than `gpc',
 * indicating a Pascal dialect.
 */
static int dialect = 0;

/* Name of Pascal dialect requested.
 */
static char *dialectname;

char *dialect_options[] =
{
  "",                           /* gnu-pascal */
  "-fstandard-pascal",
  "-fextended-pascal",
...
static void safe_exit PROTO ((int));
static void init_automake_temp_file_name PROTO ((int*, char***));
static void init_automake_temp_file PROTO ((int, char**));
static void check_amtmpfile_switch PROTO ((void));
static char * add_automake_files PROTO ((char *));
...
#ifdef GPC
/* config.h can define GPC1_SPEC to provide extra args to gpc1
   or extra switch-translations.  */
#ifndef GPC1_SPEC
#define GPC1_SPEC ""
#endif
...
#ifdef GPC
static char *gpc1_spec = GPC1_SPEC;
#endif
...
static struct compiler default_compilers[] =
{
  ...
  {".fpp", {"#Fortran"}},
  {".pas", {"#Pascal"}}, {".p", {"#Pascal"}}, {".pp", {"#Pascal"}}, {".dpr", {"#Pascal"}},
     ...
     char ***argvp;
{
  ...
  char **argv = *argvp;
#ifdef GPC
  char **newv = (char **) xmalloc ((argc + 5) * 2 * sizeof (char *));
#else
  ...
  while (i < newindex)
    {
      if (library != 0
          && newv[i][2] == '\0'
          && (char *) strchr ("hv", newv[i][1]) != NULL
          && (newindex == 2
              || (newindex == 3
                  && strncmp (newv[1], "-famtmpfile=", 12) == 0)))
        {
          /* If they only gave us `-v' or `-h', don't try to link
             in libgpc, and forget about dialect.  */
          library = 0;
          dialect = 0;
        }
      if (library != 0
          && ((newv[i][2] == '\0'
               && (char *) strchr ("cSEM", newv[i][1]) != NULL)
              || strcmp (newv[i], "-MM") == 0
              || strcmp (newv[i], "-fsyntax-only") == 0))
        {
      ...
      if (dialect
          && (strcmp (newv[i], "-fstandard-pascal") == 0
              || strcmp (newv[i], "-fextended-pascal") == 0
              || strcmp (newv[i], "-fobject-pascal") == 0
              || strcmp (newv[i], "-fborland-pascal") == 0
              || strcmp (newv[i], "-fdelphi") == 0
              || strcmp (newv[i], "-fpascal-sc") == 0))
        {
          fatal ("Pascal dialect options only allowed if invoked as `gpc'.");
        }
      i++;
    }

  /* Add Pascal dialect specific options */
  if (dialect)
    newv[newindex++] = dialect_options[dialect];
#endif /* GPC */
...
#ifdef GPC
  INIT_STATIC_SPEC ("gpc1",                     &gpc1_spec),
#endif /* GPC */
 ...
 * This is to circumvent broken shells without quoting.
 * (Please guess under what "OS" they exist ...)
 */
void
  ...
  while (i < argc)
    {
      ...
      if (quoted && i + 1 < argc)
        {
          int j;
          argv [i] = concat (argv [i], " ", argv [i + 1], NULL_PTR);
          for (j = i + 2; j < argc; j++)
            argv [j - 1] = argv [j];
          argc--;
        }
...
/* Hack the command line to implicitly #define __OS_DOS__
 * if one of MSDOS, _WIN32, or __EMX__ is defined.
 */
static void
hack_predefines ()
{
  char *p = cpp_predefines;
  int os_dos = 0;
  while (*p && ! os_dos)
    {
      while (*p && *p != '-')
  ...
  if (os_dos)
    {
      char *new_predefines = concat (cpp_predefines,
                                     " -D__OS_DOS__=1", NULL_PTR);
      cpp_predefines = new_predefines;
    }
}
#endif /* GPC */
...
execute ()
{
  ...
  for (n_commands = 1, i = 0; i < argbuf_index; i++)
    if (strcmp (argbuf[i], "|") == 0)
      {                         /* each command.  */
#ifndef __EMX__
#if defined (MSDOS) || (defined (_WIN32) && ! defined (__CYGWIN32__)) || defined (OS2) || defined (VMS)
     ...
     char **argv;
{
...
#ifdef GPC
  gcc_exec_prefix = getenv ("GPC_EXEC_PREFIX");
  if (! gcc_exec_prefix)
#endif
...
#ifdef GPC
  if (argc == 1
      || (argc == 2 && strncmp (argv[1], "-famtmpfile=", 12) == 0))
    print_message_and_exit();
#endif /* GPC */
  ...
  for (i = 1; i < argc; i++)
   ...
   && !defined(__EMX__) && !defined(DJGPP)
        /* This code would break -Zexe under EMX */
        ...
        */
              argv[i] = convert_filename (argv[i], !(have_c || have_M));
              if (p[1] == 0)
...
#ifdef GPC
          case '3':
            value = do_spec_1 (gpc1_spec, 0, NULL_PTR);
            if (value != 0)
              return value;
            break;
#endif /* GPC */
 ...
 */
static void
init_automake_temp_file_name (argcp, argvp)
     int *argcp;
     char ***argvp;
{
  int argc = *argcp;
  ...
  for (i = 1; i < argc; i++)
    {
      if (strncmp (argv[i], "-famtmpfile", 11) == 0 || strncmp (argv[i], "--amtmpfile", 11) == 0)
        return;
    }

  /* Only reached if we are at top level (and not called by AutoMake).
   */

  /* Choose a name for the AutoMake temp file.
   */
  automake_temp_file_name = concat (choose_temp_base(), "", NULL_PTR);

  /* Add an implicit command line argument `-famtmpfile=ccXXXXXX.gpc'.
   */
  argc++;
  argv = (char **) xmalloc (argc * sizeof (char *));
  argv[0] = *argvp[0];
  argv[1] = concat ("-famtmpfile=", automake_temp_file_name, NULL_PTR);
  if (argc > 2)
    memcpy (argv + 2, *argvp + 1, (argc - 2) * sizeof (char *));
  *argcp = argc;
  *argvp = argv;
}
     ...
     char **argv;
{
  ...
  /* automake_temp_file_name is left to be NULL by
     init_automake_temp_file_name() if an `--amtmpfile' option is
     given, i.e., this is a recursive automake compile. In this
     case, we don't want to initialize the automake temp file. */
  if (!automake_temp_file_name)
    return;

  automake_temp_file = fopen (automake_temp_file_name, "wt");
  if (!automake_temp_file)
    fatal ("cannot create automake temp file `%s'", automake_temp_file_name);

  fprintf (automake_temp_file, "#gpc: %s\n", argv[0]);
  fprintf (automake_temp_file, "#cmdline:");
  for (i = 1; i < argc; i++)
    {
      int j, ignore = 0;
      for (j = 0; j < n_infiles && ! ignore; j++)
        if (strcmp (argv[i], infiles[j].name) == 0)
          ignore = 1;
      if (! ignore
          && strcmp (argv[i], "-c") != 0
          && strcmp (argv[i], "-S") != 0
          ...
          && strncmp (argv[i], "--uses", 6) != 0
          && strncmp (argv[i], "-fuses", 6) != 0
          && strncmp (argv[i], "--autolink", 10) != 0
          && strncmp (argv[i], "-fautolink", 10) != 0
          && strncmp (argv[i], "--automake", 10) != 0
          && strncmp (argv[i], "-fautomake", 10) != 0
          && strncmp (argv[i], "--autobuild", 11) != 0
          && strncmp (argv[i], "-fautobuild", 11) != 0
          && strncmp (argv[i], "-famtmpfile", 11) != 0)
        fprintf (automake_temp_file, " %s", argv[i]);

      /* With "-o", do not pass the given file name either.
       */
      if (strcmp (argv[i], "-o") == 0)
        i++;
    }
  fprintf (automake_temp_file, "\n");
...
check_amtmpfile_switch ()
{
  int is_pascal = 0;
  int i = 0;
  while (i < n_infiles && ! is_pascal)
    {
      char *p;
      char *input_basename = infiles[i].name;
      int basename_length;
      ...
      if (strcmp (p, ".pas") == 0 || strcmp (p, ".p") == 0
  ...
  if (! is_pascal)
    {
      ...
      while (i < n_switches)
        {
          if (strncmp (switches[i].part1, "famtmpfile", 10) == 0)
            {
              if (i < n_switches - 1)
                memmove (switches + i, switches + i + 1,
                         (n_switches - i - 1) * sizeof (struct switchstr));
              n_switches--;
            }
          else
            i++;
     ...
     char *explicit_link_files;
{
  ...
  int i = n_infiles;
  /* Make the buffer big enough to hold a long `gpc1' command line.
   * (@@@ What's a reasonable way to allocate the buffer dynamically?)
   */
  char buffer [10240];

  if (!automake_temp_file_name)
    abort ();
  automake_temp_file = fopen (automake_temp_file_name, "rt");
  if (!automake_temp_file)
    fatal ("cannot read automake temp file `%s'", automake_temp_file_name);
  ...
  while (! feof (automake_temp_file))
    {
      ...
      if (i >= n_infiles)
        {
          ...
          free (explicit_link_files);
          explicit_link_files = new_explicit_link_files;
        }
      if (fgets (buffer, 10240, automake_temp_file) == NULL_PTR)
        break;
      l = strlen (buffer);
      if (l > 0 && buffer[l - 1] == '\n')
        buffer[l - 1] = 0;
      ...
      /* Process an implicit `-o' switch, but only if
       * no explicit one is present.
       */
      if (strncmp ("-o ", buffer, 3) == 0)
        {
          if (explicit_output_file)
            continue;
          else
            {
              /* Add an `-o' switch to `switches'.
               */
              struct switchstr *new_switches;
              new_switches = (struct switchstr *)
                             xmalloc ((n_switches + 1)
                                      * sizeof (struct switchstr));
              memcpy (new_switches, switches,
                      (n_switches + 1) * sizeof (struct switchstr));
              switches[n_switches].part1 = "o";
              switches[n_switches].args = (char **) xmalloc (2 * sizeof (char *));
              switches[n_switches].args[0] = save_string (buffer + 3,
                                                          strlen (buffer + 3));
              switches[n_switches].args[1] = NULL;
              switches[n_switches].live_cond = 0;
              switches[n_switches].valid = 0;
              n_switches++;
              continue;
      ...
      if (buffer[0] == '-' && buffer[1] == 'l')
        {
          /* Add the library file at the end */
          outfiles [i] = save_string (buffer, strlen (buffer));
  ...
  if (library)
    {
      int i = n_infiles;
      int have_math_library = !!(strlen (MATH_LIBRARY) > 0);
      int new_n_infiles = n_infiles + 1 + have_math_library;
      char **new_outfiles = (char **) xmalloc (new_n_infiles *
                                               sizeof (char*));
      char *new_explicit_link_files = xmalloc (new_n_infiles);
      bcopy ((char *) outfiles,
     ...
     char **argv;
{
  ...
  /* If we're called as gpc (or i386-aout-gpc), link in libgpc as well.  */
  p = argv[0] + strlen (argv[0]);
  while (p != argv[0] && p[-1] != '-' && p[-1] != '/' && p[-1] != DIR_SEPARATOR)
    --p;
  /* If called as x??, remove the x */
  if (p[0] == 'x')
    p++;
  dialectname = p;

  if (strcmp (dialectname, "gpc") == 0)
    {   /* GNU Pascal */
      library = 2;
    }
  else if (strcmp (dialectname, "pc") == 0)
    {   /* Standard (ISO 7185) Pascal */
      dialect = 1;
      library = 2;
    }
  else if (strcmp (dialectname, "epc") == 0)
    {   /* Extended (ISO 10206) Pascal */
      dialect = 2;
      library = 2;
    }
  else if (strcmp (dialectname, "opc") == 0)
    {   /* Object Pascal */
      dialect = 3;
      library = 2;
    }
  else if (strcmp (dialectname, "bpc") == 0)
    {   /* Borland Pascal */
      dialect = 4;
      library = 2;
    }
  else if (strcmp (dialectname, "dcc") == 0)
    {   /* Borland Delphi */
      dialect = 5;
      library = 2;
    }
  else if (strcmp (dialectname, "psc") == 0)
    {
      dialect = 6;
      library = 2;
    }
...
#ifdef GPC
  concat_args (&argc, argv);  /* Handle "..." args containing blanks. */
  init_automake_temp_file_name (&argc, &argv);
#endif /* GPC */
  ...
  if (*cross_compile == '0')
    {
      ...
      /* If it starts with $, assume it as absolute path, which should
         be expanded by update_path() */
  ...
  if (verbose_flag)
    {
      ...
      if (! strncmp (version_string, compiler_version, n)
        ...
        fprintf (stderr, "gpc version %s, based on gcc-%s\n",
                 GPC_VERSION_STRING, version_string);
      else
        fprintf (stderr, "gpc driver version %s, based on gcc-%s, executing gpc version %s\n",
                 GPC_VERSION_STRING, version_string, compiler_version);
#else /* EGCS */
      fprintf (stderr, "gpc version %s, based on %s\n",
               GPC_VERSION_STRING, version_string);
#endif /* EGCS */
...
#ifdef EGCS95
void
fatal PVPROTO((const char *format, ...))
#else
static void
