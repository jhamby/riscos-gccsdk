...
#define DEFAULT_WORD_SWITCH_TAKES_ARG(STR)		\
 (!strcmp (STR, "Tdata") || !strcmp (STR, "Ttext")	\
  ...
  || !strcmp (STR, "icrossdirafter") || !strcmp (STR, "isystem") \
  || !strcmp (STR, "specs"))
	  ...
	  if (commands[j].pid == pid)
	      ...
	      if (status != 0)
		{
		  if (WIFSIGNALED (status))
		    {
		      fatal ("Internal compiler error: program %s got fatal signal %d (%s)",
		    ...
		    {
		      if (WEXITSTATUS (status) != FATAL_EXIT_CODE)
			fatal ("Fatal compiler error: program %s",
			       commands[j].prog);
		      ret_code = -1;
		    }
		}
...

#if defined(HAVE_OBJECT_SUFFIX) || defined(HAVE_EXECUTABLE_SUFFIX) || defined(__riscos__)
     ...
     char **argv;
{
  ...
  for (i = 1; i < argc; i++)
	...
	{
#if defined(HAVE_OBJECT_SUFFIX) || defined(__riscos__)
