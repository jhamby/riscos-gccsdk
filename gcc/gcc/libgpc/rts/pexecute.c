...
extern void *_p_malloc (size_t);
extern void  _p_free (void *);
extern char *_p_strlcpy (char *, const char *, size_t);
extern char *_p_strscan (const char *, char);
     ...
     int flags;
{
  ...
  scmd = (char *) _p_malloc (strlen (program) + strlen (temp_base) + 6 + el);
      ...
      for (j = 0; j < len; j++)
        {
          if (temp[j] == '"')
            {
              newtemp = (char *) _p_malloc (len + 2);
              _p_strlcpy (newtemp, temp, j-1);
              newtemp [j] = '\\';
              _p_strlcpy (&newtemp [j+1], &temp [j], len-j-1);
              newtemp [len+1] = 0;
              temp = newtemp;
              len++;
              j++;
      ...
      for (i=1; argv[i]; i++)
        {
          ...
          if (_p_strscan(argv[i], '/'))
            {
              tmpname = (char *) _p_malloc (256);
