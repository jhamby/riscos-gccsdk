...
Sets are stored as bitmaps consisting of TSetElement's. They contain
bits for the set's range plus possibly padding at either or both
ends Thus, there may be unused bits in front and after the end of
otherwise contiquous bit vector that represents all the elements of
the set.
...
type
  ...
  PSet = ^TSet;
  TSet = array [0 .. MaxVarSize div SizeOf (TSetElement)] of TSetElement;

function  SetCard                (SetA : PSet; LowA, HighA : Integer) : Integer;                                                               asmname '_p_set_card';
...
function  SetLess                (SetA : PSet; LowA, HighA : Integer; SetB : PSet; LowB, HighB : Integer) : Boolean;                           asmname '_p_set_less';
function  SetIn                  (SetA : PSet; LowA, HighA, Element : Integer) : Boolean;                                                      asmname '_p_set_in';
procedure SetClear               (SetR : PSet; LowR, HighR : Integer);                                                                         asmname '_p_set_clear';
function  SetInclude             (SetR : PSet; LowR, HighR, Element : Integer) : Boolean;                                                      asmname '_p_set_include';
function  SetExclude             (SetR : PSet; LowR, HighR, Element : Integer) : Boolean;                                                      asmname '_p_set_exclude';
function  SetIncludeRange        (SetR : PSet; LowR, HighR, RangeFirst, RangeLast : Integer) : Boolean;                                        asmname '_p_set_include_range';
...
procedure SetBits                (SetA : PSet; BitLength : MedCard; RangeFirst, RangeLast : MedInt);                                           asmname '__setbits';
...
const
  ...
  TempTypeSize = BitSizeOf (LongestInt);

type
  TempType = packed array [1 .. TempTypeSize] of 0 .. BitsPerElement - 1;

const
  Log2BitsPerElement = BitSizeOf (TempType) div TempTypeSize; {:-}

{ Returns the word number in which the n'th member is in when starting from 0 }
inline function WordNumberAbs (n : Integer) : Integer;
begin
...
inline function WordNumber (n, LowA : Integer) : Integer;
begin
   WordNumber := WordNumberAbs (n) - WordNumberAbs (LowA)
end;

{ Returns the number of words needed to adjust set A to B }
inline function VectorAdjust (LowA, LowB : Integer) : Integer;
begin
...
begin
  ...
  if LeaveFirstWord >= 0 then And (SetA^[LeaveFirstWord], AllBitsSet shl BitNumber (LeaveFirst));
  for i := LeaveLastWord + 1 to WordNumber (HighA, LowA) do SetA^[i] := 0;
  if LeaveLastWord <= WordNumber (HighA, LowA) then And (SetA^[LeaveLastWord], not (AllBitsSet shl BitNumber (LeaveLast + 1)))
end;
...
begin
  ...
  if RangeFirstWord = RangeLastWord then
    ...
    begin
      if RangeFirstWord >= 0 then Or (SetR^[RangeFirstWord], AllBitsSet shl BitNumber (RangeFirst));
      for i := Max (0, RangeFirstWord + 1) to Min (RangeLastWord - 1, WordNumber (HighR, LowR)) do SetR^[i] := AllBitsSet;
      if RangeLastWord <= WordNumber (HighR, LowR) then Or (SetR^[RangeLastWord], not (AllBitsSet shl BitNumber (RangeLast + 1)))
    end
end;

function SetEmpty (SetA : PSet; LowA, HighA : Integer) : Boolean;
var i : Integer;
begin
...
begin
  if (LowA > HighB) or (LowB > HighA) then
    SetEqual := SetEmpty (SetA, LowA, HighA) and SetEmpty (SetB, LowB, HighB)
  else
    begin
      SetEqual := False;
...
begin
  if (LowA > HighB) or (LowB > HighA) then
    SetLessEqual := SetEmpty (SetA, LowA, HighA)
  else
    begin
      SetLessEqual := False;
      wa := WordNumber (HighA, LowA);
...
function SetInclude (SetR : PSet; LowR, HighR, Element : Integer) = RangeOK : Boolean;
begin
  RangeOK := (Element >= LowR) and (Element <= HighR);
  if RangeOK then Or (SetR^[WordNumber (Element, LowR)], 1 shl BitNumber (Element))
end;

function SetExclude (SetR : PSet; LowR, HighR, Element : Integer) = RangeOK : Boolean;
begin
  RangeOK := (Element >= LowR) and (Element <= HighR);
  if RangeOK then And (SetR^[WordNumber (Element, LowR)], not (1 shl BitNumber (Element)))
end;
...
begin
  if (LowA > HighB) or (HighA < LowB) then
    ...
    begin
      ...
      for i := Max (0, - a) to Min (WordNumber (HighR, LowR), WordNumber (HighB, LowB) - a) do And (SetR^[i], SetB^[i + a]);
...
begin
  SetCopy (SetA, LowA, HighA, SetR, LowR, HighR);
  a := VectorAdjust (LowB, LowR);
  for i := Max (0, - a) to Min (WordNumber (HighR, LowR), WordNumber (HighB, LowB) - a) do Or (SetR^[i], SetB^[i + a])
end;
...
begin
  SetCopy (SetA, LowA, HighA, SetR, LowR, HighR);
  a := VectorAdjust (LowB, LowR);
  for i := Max (0, - a) to Min (WordNumber (HighR, LowR), WordNumber (HighB, LowB) - a) do And (SetR^[i], not SetB^[i + a])
end;
...
begin
  SetCopy (SetA, LowA, HighA, SetR, LowR, HighR);
  a := VectorAdjust (LowB, LowR);
  for i := Max (0, - a) to Min (WordNumber (HighR, LowR), WordNumber (HighB, LowB) - a) do Xor (SetR^[i], SetB^[i + a])
end;

procedure SetBits (SetA : PSet; BitLength : MedCard; RangeFirst, RangeLast : MedInt);
begin
  if not SetIncludeRange (SetA, 0, BitLength, RangeFirst, RangeLast) then
    RuntimeError (309) { range error in set constructor }
end;
