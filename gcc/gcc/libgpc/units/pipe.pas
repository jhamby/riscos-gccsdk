...
(*$local W-*)type PPStrings = ^TPStrings;TPStrings (Count: Cardinal) = array [1 .. Count] of ^String; (*@@two unit bug with definition in gpc.pas in programs using these two units*) (*$endlocal*)
...
function CPipe (Path : CString; CProcessParameters, ProcessEnvironment : PCStrings; PPipeStdIn, PPipeStdOut, PPipeStdErr : PInteger; ChildProc : TProcedure) : Integer; asmname '_p_pipe';
...
begin
  ...
  with Process^ do
    begin
      while Seconds <> 0 do
        begin
          if DoWaitPID (False) = PID then Exit;
          Sleep (1);
          Dec (Seconds)
        end;
      if Signal <> 0 then Dummy := Kill (SignalPID, Signal);
      if not Wait or (DoWaitPID (True) <= 0) then WaitPipeProcess := False
    end;
  Dispose (Process)
end;
...
begin
  with FileProcess do
    begin
      ...
      if ProcessPtr^.OpenPipes = 0 then Dummy := WaitPipeProcess (ProcessPtr)
    end;
  Dispose (@FileProcess)
end;
...
procedure Pipe (var ToInputFile, FromOutputFile, FromStdErrFile : AnyFile; (*@@fjf265 const*) ProcessName : String; protected var Parameters : TPStrings; ProcessEnvironment : PCStrings; var Process : PPipeProcess; ChildProc : TProcedure);
var
  ParameterCount, i : Cardinal;
  PipeStdIn, PipeStdOut, PipeStdErr : Integer;
  PPipeStdIn, PPipeStdOut, PPipeStdErr : PInteger;
  PID : Integer;
...
begin
  ...
  if ProcessName = ''
    then ProcessPath := ''
    else ProcessPath := FSearchExecutable (ProcessName, GetEnv (PathEnvVar));
  if (ProcessName <> '') and (ProcessPath = '')
    then PID := - 1
    else PID := CPipe (ProcessPath, PCStrings (@CProcessParameters), ProcessEnvironment, PPipeStdIn, PPipeStdOut, PPipeStdErr, ChildProc);
  if PID <= 0 then
    begin
      IOErrorCString (EFork, ProcessName);
      Exit
    end;
  New (PProcess);
  PProcess^.PID := PID;
...
function CPipe (Path : CString; CProcessParameters, ProcessEnvironment : PCStrings; NameStdIn, NameStdOut, NameStdErr : CString; ChildProc : TProcedure) : Integer; asmname '_p_pipe';
...
begin
  with PipeData do
    begin
      ...
      if PipeProcess.PID < 0 then IOErrorCString (ESpawn, ProcName);
      Dispose (ProcName);
      Dispose (Path);
      for i := 0 to ParameterCount do Dispose (CProcessParameters^[i]);
      Dispose (CProcessParameters);
      Dummy := WaitPipeProcess (@PipeProcess);
  ...
  with FileProcess do
    begin
      ...
      if PipeDataPtr^.PipeProcess.OpenPipes = 0 then
        begin
          Dummy := WaitPipeProcess (@PipeDataPtr^.PipeProcess);
          Dispose (PipeDataPtr)
        end
    end;
  Dispose (@FileProcess)
...
begin
  ...
  if (ProcessName <> '') and (ProcessPath = '') then
    begin
      IOErrorCString (ESpawn, ProcessName);
      Exit
    end;
  New (PipeData);
  with PipeData^ do
