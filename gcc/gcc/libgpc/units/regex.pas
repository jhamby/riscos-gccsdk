...
the literature, or the info of the rx library, or man pages of
programs like grep(1) and sed(1).

Basic          Extended       Meaning
`.'            `.'            matches any single character
`[aei-z]'      `[aei-z]'      matches either `a', `e', or any
                              character from `i' to `z'
`[^aei-z]'     `[^aei-z]'     matches any character but `a', `e', or
                              `i' .. `z' To include in such a list
                              the characters `]', `^', or `-', put
...
`^'            `^'            matches the empty string at the
                              beginning of a line
`$'            `$'            matches the empty string at the end of
                              a line
...
`\{N\}'        `{N}'          matches exactly N occurences of the
                              preceding expression (N is an integer
                              number)
`\{M,N\}'      `{M,N}'        matches M to N occurences of the
                              preceding expression (M and N are
                              integer numbers, M <= N)
`A B'          `A B'          matches A followed by B (A and B are
                              regular expressions)
...
`\( \)'        `( )'          forms a subexpression, to override
                              ...
                              references
`\7'           `\7'           matches the 7'th subexpression
                              (counted by their start in the regex),
                              where 7 is a number from 1 to 9 ;-).
                              *Please note:* using this feature can
                              be *very* slow or take very much
                              memory (exponential time and space in
                              the worst case, if you know what that
                              means...).
`\'            `\'            quotes the following character if it's
                              special (i.e. listed above)
...
replacement string can reference the whole matched expression
with `&' or `\0', the 7th subexpression with `\7' (just like in
the regex itself, but using it in replacements is not slow), and
the 7th subexpression converted to upper/lower case with `\u7' or
`\l7', resp. (which also works for the whole matched expression with
`\u0' or `\l0'). A verbatim `&' or `\' can be specified with `\&'
or `\\', resp.
...
type
  { The type used by the routines of the `RegEx' unit to store
    ...
    RegMatch, From and Length are only used internally.
    SubExpressions can be read after `NewRegEx' and will contain the
    number of parenthesized subexpressions. Error should be checked
    after `NewRegEx'. It will be `nil' when it succeeded, and
    contain an error message otherwise. }
  RegExType = record
    RegEx, RegMatch : Pointer;
    From, Length,
...
function  RegExPosFrom (const Expression : String; ExtendedRegEx, CaseInsensitive : Boolean; const s : String; From : Integer) : Integer; asmname '_p_regexposfrom';
...
procedure NewRegEx (var RegEx : RegExType; const Expression : String; ExtendedRegEx, CaseInsensitive, NewLines : Boolean); asmname '_p_newregex';
...
function  CharSet2RegEx (const Characters : CharSet) : TString; asmname '_p_CharSet2RegEx';
...
procedure CNewRegEx       (var RegEx : RegExType; Expression : CString; ExpressionLength : Integer; ExtendedRegEx, CaseInsensitive, NewLines : Boolean); asmname '_p_new_regex';
function  CMatchRegExFrom (var RegEx : RegExType; aString : CString; StrLength : Integer; NotBeginningOfLine, NotEndOfLine : Boolean; From : Integer) : Boolean; asmname '_p_match_regex_from';
