...
static unsigned int alfversion,	/* Version of ALF used by library */
...
read_libchunks (libheader * lp)
{
  ...
  if (dirybase == NIL || vsrnbase == NIL || symtbase == NIL)
    {
      error ("Fatal: Out of memory in 'read_libchunks' reading '%s'",
	     lp->libname);
      return FALSE;
    }
...
*/
static chunkindex *
find_chunkentry (unsigned int index)
{
...
*/
bool
addto_liblist (const char *name, unsigned int *filebase,
...
isoldlib (void)
{
  ...
  n = 1;
  while (n <= chunkcount
	 && (cp->chunkclass != LIB_XXXX || cp->chunktype != LIB_VSRN))
    {
      cp++;
      n += 1;
    }
...
scan_libchunkhdr (libheader * lp)
{
  ...
  for (n = 1; n <= chunkcount; n++)
	...
	{
	  error ("Error: Chunk file header in library '%s' is corrupt",
		 lp->libname);
	  return FALSE;
	}
      if (start != 0
	  && (cp->chunkclass == LIB_XXXX || cp->chunkclass == OFL_XXXX))
	{
	  switch (cp->chunktype)
	    {
	    case LIB_VSRN:
	    ...
	    default:
	      error
		("Error: Entry %d in chunk index of library '%s' is corrupt ",
		 n, lp->libname);
	      return FALSE;
	    }
	}
  ...
  if (dirystart == 0 || !datafound)
    {
      error
	("Error: Library '%s' does not contain the correct 'chunk' types. Is it corrupt?",
	 lp->libname);
      return FALSE;
    }
  else if (symtstart == 0)
    {
      error
	("Error: Library '%s' does not appear to be an object code library",
	 lp->libname);
      return FALSE;
    }
  return TRUE;
...
*/
static char *
find_membername (unsigned int index)
{
  typedef struct
  {
...
*/
static bool
add_libsymbol (libheader * fp, unsigned int index, char *name)
{
  char *memname;
  libentry *lp;
  chunkindex *hp;
  ...
  if (freeliblist != NIL)
    ...
    {
      error ("Fatal: Out of memory in 'add_libsymbol' adding '%s'",
	     fp->libname);
    }
  hashval = hash (name);

  {
    char *newname = check_libedit (memname, name, hashval);
...

  {
    ...
    while (*tree != NIL)
      {
	int compval =
	  opt_case ? stricmp (name, (*tree)->libname) : strcmp (name,
								(*tree)->
								libname);
...
scan_libsymt (libheader * lp)
{
  ...
  while (sp < symtend)
	...
	{
	  error ("Error: 'OFL_SYMT' entry size in '%s' is too large",
		 lp->libname);
	  return FALSE;
	}
      if (!add_libsymbol (lp, sp->chunkindex, &sp->entryname))
	return FALSE;
...
open_library (libheader * lp)
{
  ...
  if (lp->libgotsyms)
    {				/* Aready read library symbol table */
      current_lib = lp;
      current_libhash = hash (lp->libname);
      current_libsyms = &lp->librarysyms;
      ok = TRUE;
      if (libstart == NIL)
	{			/* Reading library members from disk */
  ...
  if (libstart == NIL)
    {				/* Library is not in memory */
      if (!open_object (lp->libname))
	return FALSE;
      if (!read_libchunkhdr (lp))
	return FALSE;
      if (!scan_libchunkhdr (lp))
	return FALSE;
      if (!read_libchunks (lp))
	return FALSE;
    }
  else
...
close_library (libheader * lp)
{
  int n;
  if (lp->libase == NIL)
    close_object ();		/* Were reading library members from disk */
...
{
  loadlist *lp;
  if ((lp = allocmem (sizeof (loadlist))) == NIL)
...
*/
bool
load_wholelib (const char *libname, unsigned int filesize)
{
  int n, chunkcount;
  chunkindex *cp;
  chunkhdr *chp;
  ...
  for (n = 1; n <= chunkcount; n++)
      ...
      if (start != 0)
	{
	  if (cp->chunkclass == LIB_XXXX)
		...
		default:
		  error
		    ("Error: Chunk file header in Library '%s' is corrupt",
		     libname);
		  return FALSE;
		}
	    }
  ...
  for (n = 1; n <= chunkcount; n++)
	...
	{
	  strncpy (objectname, find_membername (n - 1),
		   sizeof (objectname) - 1);
	  objectname[sizeof (objectname) - 1] = '\0';
	  if (opt_verbose)
	    error ("Drlink:   Extracting '%s'...", objectname);
	  if (!extract_member (cp))
	    return FALSE;
	}
      cp++;
    }
