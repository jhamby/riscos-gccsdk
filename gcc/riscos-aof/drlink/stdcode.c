...
*/
void
get_hdrcode (segtype hdrtype, unsigned int **hdrstart, unsigned int *hdrsize)
{
  unsigned int *p;
...
*/
void
setup_binhdr (unsigned int *code, unsigned int startaddr)
{
...
void
setup_aifhdr (unsigned int *code, unsigned int startaddr)
{
  ...
  base->aif_rwsize =
    image_rwlimit->symtptr->symtvalue - image_rwbase->symtptr->symtvalue -
    zisize;
  base->aif_zisize = zisize;
  base->aif_debugsize = debugsize;
  if (debugsize > 0)
    base->aif_debugtype = DBG_SOURCE;
  base->aif_workspace = workspace;
  if (got_32bitapcs && !got_26bitapcs)
    base->aif_amode = ADDRESS_32BIT;
  if (opt_strongarm)
    {				/* Set 'code is StrongARM compatible' flag in flag byte 1 */
      base->aif_amode |= STRONGBIT;
    }
  base->aif_entrypoint +=
    (startaddr - imagebase - BRENTRY_OFFSET - PCMODIFIER) >> 2;
  if (zisize == 0)
    {				/* No zero-init area. Change zero-init BL to NOOP */
  ...
  if (imagetype == AIF)
    ...
    {				/* Relocatable image */
      base->aif_selfreloc +=
	(relocaddr - imagebase - BRELOC_OFFSET - PCMODIFIER) >> 2;
    }
}
...
*/
void
setup_modcode (unsigned int *code)
{
  modheader *base;
  base =
    COERCE (COERCE (code, char *) + sizeof (modcode) - sizeof (modheader),
	    modheader *);
