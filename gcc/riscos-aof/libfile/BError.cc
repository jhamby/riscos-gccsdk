...
BError::BError(ErrorCode a_err, BString a_file, int a_line)
{
  m_err = a_err;
  m_file = a_file;
  m_line = a_line;
...
BError::BError(BString a_strerr, BString a_file, int a_line)
  : m_strErr(a_strerr)
{
  m_err = Generic;
  m_file = a_file;
...
ostream &operator<<(ostream &out, const BError &a_err)
{
#ifdef DEBUG
