...
public:
  ...
  {
    for (int j = 0; j < List<T*>::m_size; j++)
      for (int i = 1; i < List<T*>::m_size; i++)
	{
	  const T *pt1 = *((T **) List<T*>::m_data[i]);
	  const T *pt2 = *((T **) List<T*>::m_data[i - 1]);
	  if (lessThan (pt1, pt2))
	    {
	      T **tmp = (T **) List<T*>::m_data[i];
	      List<T*>::m_data[i] = List<T*>::m_data[i - 1];
	      List<T*>::m_data[i - 1] = tmp;
	    }
	}
  }
...
public:
  ...
  List < T > *the_list ()
  {
    return ((List < T > *) Const_listiter<T>::m_list);
  }

  void insert_next (const T & t)
  {
    Const_listiter<T>::m_list->insert (Const_listiter<T>::m_pos);
    (Const_listiter<T>::m_list->m_data)[Const_listiter<T>::m_pos] = new T (t);

    Const_listiter<T>::m_list->inserted (Const_listiter<T>::m_pos);
  }

  void insert_prev (const T & t)
  {
    Const_listiter<T>::m_list->insert (Const_listiter<T>::m_pos - 1);
    (Const_listiter<T>::m_list->m_data)[Const_listiter<T>::m_pos - 1] = new T (t);

    Const_listiter<T>::m_list->inserted (Const_listiter<T>::m_pos - 1);
  }

  int replace_next (const T & t)
  {
    int res = 0;
    if (Const_listiter<T>::m_pos < Const_listiter<T>::m_list->length ())
      {
	*((T *) ((Const_listiter<T>::m_list->m_data)[Const_listiter<T>::m_pos])) = t;
	res = 1;
      }
    return (res);
  }
  ...
  {
    int res = 0;
    if (Const_listiter<T>::m_pos > 0)
      {
	*((T *) ((Const_listiter<T>::m_list->m_data)[Const_listiter<T>::m_pos - 1])) = t;
	res = 1;
      }
    return (res);
  }
  ...
  int remove_next ()
  {
    ...
    if (Const_listiter<T>::m_pos < Const_listiter<T>::m_list->length ())
      {
	Const_listiter<T>::m_list->remove (Const_listiter<T>::m_pos);
	res = 1;
      }
    return (res);
  }
  ...
  int remove_prev ()
  {
    ...
    if (Const_listiter<T>::m_pos > 0)
      {
	Const_listiter<T>::m_list->remove (Const_listiter<T>::m_pos - 1);
	res = 1;
      }
    return (res);
  }
  ...
  int remove_next (T & t)
  {
    ...
    if (Const_listiter<T>::m_pos < Const_listiter<T>::m_list->length ())
      {
	t = *((T *) ((Const_listiter<T>::m_list->m_data)[Const_listiter<T>::m_pos]));
	Const_listiter<T>::m_list->remove (Const_listiter<T>::m_pos);
	res = 1;
      }
    return (res);
  ...
  int remove_prev (T & t)
  {
    ...
    if (Const_listiter<T>::m_pos > 0)
      {
	t = *((T *) ((Const_listiter<T>::m_list->m_data)[Const_listiter<T>::m_pos - 1]));
	Const_listiter<T>::m_list->remove (Const_listiter<T>::m_pos - 1);
	res = 1;
      }
    return (res);
