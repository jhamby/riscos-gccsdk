/****************************************************************************
 *
 * $Source: /usr/local/cvsroot/gccsdk/unixlib/source/sys/exec1.c,v $
...
#ifdef EMBED_RCSID
static const char rcs_id[] = "$Id: exec1.c,v 1.1.1.1 2000/07/15 14:52:33 nick Exp $";
#endif
...
execve (const char *execname, char **argv, char **envp)
{
  ...
  *command_line ++ = ' ';
  ...
  if (argv[0])
    for (x = 1; argv[x]; x++)
      {
        command_line = stpcpy (command_line, argv[x]);
        *command_line ++ = ' ';
      }
  command_line[-1] = '\0';
...
#endif
  /* XXX Do we really need to copy the process arguments above because
     the child process might as well just re-translate the command
     line arguments.  */

  /* We enter here if exec was called after a vfork, which is
     usually the case.  */

  /* Re-create the process's argv and envp with the arguments and
     environment from this function.  First free the old argument
     and environment vectors.  From now on, this is the point
     of no return.  */
  ...
  /* Count new argument vector length.  */
  for (x = 0; argv[x]; x++)
     ;
  process->argc = x;
  ...
  process->argv = (char **) malloc (process->argc + 1);
  process->envp = (char **) malloc (process->envc + 1);
  if (process->argv == NULL || process->envp == NULL)
    __exit_no_code ();
  /* Copy entries from the environment and argument vectors.  */
  for (x = 0; x < process->argc; x++)
    {
      process->argv[x] = strdup (argv[x]);
