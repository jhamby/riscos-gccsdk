...
#ifdef EMBED_RCSID
static const char rcs_id[] = "$Id: fcrypt.c,v 1.2.2.1 2002/02/07 10:19:30 admin Exp $";
#endif
...
/* memset() is optimised for high performance */
#undef	BUILTIN_CLEAR
#undef	BZERO
#define	MEMSET

/* Optimisations for other machines, which don't help the ARM */
#undef	FDES_4BYTE
#undef	FDES_8BYTE
#undef	BRAINDEAD6
...
static void XForm (sbpb24 saltvalue)
{
    union
    {
       sbpb24 b[2];
...
char *fcrypt (const char *pw, const char *salt)
{
    ...
    saltvalue = salt0[(unsigned int)iobuf[0]] | salt1[(unsigned int)iobuf[1]];
...
static fbpb4 lookupS (unsl tableno, sbpb6R t6bits)
{
...
static void init (unsl tableno, sbpb24 *lowptr, sbpb24 *highptr)
{
    ...
    for (j = 0; j < 64; j++)
    {
       ...
       for (i = 0; i < 24; i++)
       {
           lowptr[j] |= (sbpb24)(tmpE[i]) << i;
       }
       for (k = 0, i = 24; i < 48; i++, k++)
       {
           highptr[j] |= (sbpb24)(tmpE[i]) << k;
       }
