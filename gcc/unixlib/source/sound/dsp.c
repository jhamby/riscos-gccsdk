...
static _kernel_oserror *set_defaults(int format, int size, int frequency)
{
...
void *__dspopen (struct __unixlib_fd *file_desc, const char *file, int mode)
{
  ...
  if ((err = set_defaults(2, BUFFER_SIZE, 44100)) ||
      ...
      case SNDCTL_DSP_SETFMT & 0xffff:
        {
          ...
          if (format == AFMT_MU_LAW || format == AFMT_S16_LE)
            {
               if (!set_defaults((format == AFMT_MU_LAW) ? 1 : 2, 0, 0))
                 {
                   return 0;
                 }
            }
