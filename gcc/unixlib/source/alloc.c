...
/*
   ...
   Changes that need sending to Doug.

   Check calculated malloc size has not wrapped around. For example,
   malloc (2*32-1) used to return a malloc'd area of 8 bytes.
...
#ifndef HAVE_MREMAP
#if defined(INTERNAL_LINUX_C_LIB) || defined(__riscos)
...
#endif
{
  ...
  cp = (char *)mremap((char *)p - offset, size + offset, new_size, MREMAP_MAYMOVE);
...
#endif
{
  ...
  if (nb + alignment + MINSIZE < bytes)
    return 0;  /* Fail when request size wraps around */
