...
spr8c_map_rgb_color(gx_device * pdev, const gx_color_value cv[])
{
	...
	gx_color_value r, g, b, rc, gc, bc;
	r = cv[0]; g = cv[1]; b = cv[2];
    rc = r * max_value / gx_max_color_value;
    gc = g * max_value / gx_max_color_value;
    bc = b * max_value / gx_max_color_value;
