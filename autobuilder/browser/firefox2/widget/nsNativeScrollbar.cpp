...
extern "C" {
  int ChoX11_SetScroll(void *ref, Window window, Bool horizontal, unsigned int view, unsigned int size, unsigned int max);

  extern void (*ChoX11_ScrollHandler)(void *, int);
}


static inline void BoundsCheck(PRInt32 low, PRUint32& value, PRUint32 high)
{
...
NS_IMPL_ISUPPORTS_INHERITED1(nsNativeScrollbar, nsWidget, nsINativeScrollbar)
...
{
  printf("native scrollbar constructor: %p\n", (void *)this);

  ChoX11_ScrollHandler = nsNativeScrollbar::ScrollEvent;
...
{
  printf("native scrollbar destroy: %p\n", (void *)this);

  ChoX11_SetScroll(NULL, mParentWindow, mHorizontal, 0, 0, 0);
...
// 
void
nsNativeScrollbar::DoScrollAction(/*ControlPartCode part*/ int part)
{
...
void nsNativeScrollbar::ScrollEvent(void *ref, int newPos) {
  nsNativeScrollbar *scrollbar = (nsNativeScrollbar *)ref;

  scrollbar->UpdateContentPosition((scrollbar->mVisibleImageSize + scrollbar->mMaxValue) * 10000 / newPos);
}


NS_IMETHODIMP
nsNativeScrollbar::SetScroll(void)
{
  //if (mVisibility != VisibilityFullyObscured && mVisibleImageSize
  printf("%d %d %d %d %d %d\n", mVisibility != VisibilityFullyObscured, mBounds.x, mBounds.y, mBounds.width, mBounds.height, mMapped);

  ChoX11_SetScroll(this, mParentWindow, mHorizontal, mValue, mVisibleImageSize, mMaxValue);
...
printf("SetMaxRange: %p %d %d\n", (void *)this, aEndRange, mHorizontal);
...
printf("set position: %p %d %d %d\n", (void *)this, aPos, mHorizontal, mVisibility);
...
{
  if ((PRInt32)aSize < 0) aSize = 0;

printf("set view size: %p %d %d %d\n", (void *)this, aSize, mHorizontal, mVisibility);
...
// GetNarrowSize
//
// Always zero, since the RISC OS scrollbars aren't part of the main frame
//
NS_IMETHODIMP
nsNativeScrollbar::GetNarrowSize(PRInt32* outSize)
{
  if (*outSize) return NS_ERROR_FAILURE;
  *outSize = 0;
  return NS_OK;
}
                              ...
                              nsIScrollbarMediator* inMediator)
{
  ...
  if (mContent) {
    ...
    mHorizontal = orient.Equals(NS_LITERAL_STRING("horizontal"));

//printf("set content: %d %p\n", mHorizontal, (void *)this);
  }

  //if (!ChoX11_SetScroll(mParentWindow, mHorizontal, mValue, mVisibleImageSize, mMaxValue)) mAttached = PR_TRUE;

#if 0
  nsIWidget *toplevel = this;
  nsCOMPtr<nsIWidget> parent = nsnull;

  do {
    parent = dont_AddRef(toplevel->GetParent());
