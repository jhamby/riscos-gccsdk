...
===================================================================
--- gcc/config/arm/arm.c	(revision 221600)
...
+    {
+      /* Unless the option -m(no-)apcs-stack-check is explicitely set, we have
+	 it default set for APCS-32 code unless we're generating module code
+	 (-mmodule) as this is generally using a non-chunk stack anyway.  */
+      if ((target_flags_explicit & MASK_APCS_STACK) == 0)
+	{
+	  if (!TARGET_MODULE)
...
+      if (arm_abi == ARM_ABI_APCS || arm_abi == ARM_ABI_APCS32)
...
+	     This may be an indication of a bug somewhere else or more likely it's
+	     just not appropriate.  */
+	  bool need_reloc = false;
+
+	  insn = emit_insn (gen_pic_load_addr_32bit (reg, orig));
+
+	  /* Don't do relocation based on PIC register when our
+	     symbol is a function or read-only data, i.e. do this only for
+	     global/local writable variables.  */
+	  if (SYMBOL_REF_ANCHOR_P (orig))
+	    {
+	      struct object_block *block = SYMBOL_REF_BLOCK (orig);
+
+	      /* A section anchor is generated when the compiler lumps together a
+	         number of global variables into a block that can be accessed via
+	         a single address. The section anchor is the label that defines
+	         the start of such a block. We already know by its nature that
+	         it exists in the global data area, so we don't have to check for
+	         that, but we do need to know whether it's writeable or read only.
+	         NOTE: a data.rel.ro section can be marked writeable.  */
+	      gcc_assert (block);
+	      if ((block->sect->common.flags & SECTION_WRITE) &&
+		 !(block->sect->common.flags & SECTION_RELRO))
+		need_reloc = true;
+	    }
+	  else
+	    {
...
+      if (arm_abi == ARM_ABI_APCS32
+	  && (!optimize
+	      || (cfun->calls_alloca && !TARGET_APCS_STACK)))
+	{
