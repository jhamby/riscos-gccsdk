...
#define isxdigit local_isxdigit

static __inline int
local_isdigit (const char x)
{
  return (x >= '0' && x <= '9') ? 1 : 0;
}
#define isdigit local_isdigit

static __inline char
local_toupper (const char x)
{
  return (x >= 'a') ? x - 26 + 7 : x;
}
#define toupper local_toupper

static __inline int
local_islower (const char x)
{
  return (x >= 'a' && x <= 'z') ? 1 : 0;
}
#define islower local_islower

static __inline size_t
local_strnlen (const char *s, size_t maxlen)
{
  size_t r;
...
do_div_p (unsigned long long *a, int b)
{
  unsigned long long rslt = *a % b;
  *a = *a / b;
  return rslt;
}
#define do_div(a, b) do_div_p (&a, b)
