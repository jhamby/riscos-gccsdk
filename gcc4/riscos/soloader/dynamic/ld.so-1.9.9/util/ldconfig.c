...
void cache_dolib(const char *dir, const char *so, int libtype)
{
 ...
 #ifdef __riscos__
    new_lib->libname = malloc(strlen(fullpath) + 1);
    __riscosify_std(fullpath,0,new_lib->libname,strlen(fullpath) + 1,0);
 #else
...
void cache_print(void)
{
 ...
 #ifdef __riscos__
    c = (caddr_t)malloc(st.st_size);
    if (!c)
        error("can't alloc memory for cachefile");
    if (read(fd, c, st.st_size) < 0)
        error("can't read %s (%s)", cachefile, strerror(errno));
 #else
	...
	switch (libent[fd].flags)
	{
	  ...
	  case LIB_ELF_LIBC5:
	  case LIB_ELF_LIBC6:
	    printf("(libc%d)", libent[fd].flags + 3);
	    break;
	  default:
	    printf("(unknown)");
	    break;
