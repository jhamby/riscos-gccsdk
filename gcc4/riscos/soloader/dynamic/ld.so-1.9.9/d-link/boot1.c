 ...
 * slot of the GOT, and as it turns out, %ebx points to the GOT when
...
/*  lpnt = (int *) (tpnt->dynamic_info[DT_PLTGOT] + load_addr);*/
  ...
  int i;
    ...
    tpnt->ppnt = ppnt = (Elf32_Phdr *) (load_addr + epnt->e_phoff);
...
/*	  _dl_fdprintf(2, "%s: can't open file '%s'\n", _dl_progname,
  ...
  _dl_brkp = (unsigned int *) _dl_find_hash("___brk_addr", NULL, NULL, NULL, 0);
  if (_dl_brkp) *_dl_brkp = brk_addr;

#ifdef IBCS_COMPATIBLE
  _dl_envp = (unsigned int *) _dl_find_hash("_environ", NULL, NULL, NULL, 0);
#else
  _dl_envp = (unsigned int *) _dl_find_hash("__environ", NULL, NULL, NULL, 0);
#endif
...
#undef DL_DEBUG
	...
	_dl_fdprintf(2, ": ");
	if (!_dl_atexit)
	  _dl_fdprintf(2, "The address of atexit () is 0x0.");
