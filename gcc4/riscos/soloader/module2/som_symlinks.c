...
static __attribute__((noinline)) _kernel_oserror *read_link_file_info(const char *filename, int *filetype, int *type)
{
...
FILE *file = NULL;
  ...
  if ((err = som_alloc(MAX_FILENAME_LEN, (void **)(void *)&file_out)) != NULL)
    goto error;
  if ((err = som_alloc(MAX_FILENAME_LEN, (void **)(void *)&buffer)) != NULL)
    goto error;
