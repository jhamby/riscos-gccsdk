...
register_dynamic_loader (runcom_state * state)
{
  ...
  if (state->elf_loader_handle != 0)
    {
      if ((err =
	   som_query_object (state->elf_loader_handle, &objinfo,
	  ...
	  if (phdr->p_type == PT_LOAD)
	      ...
	      if ((phdr->p_flags & PF_W) != 0)
		{
		  objinfo.public_rw_ptr =
		    state->elf_loader.base_addr + phdr->p_vaddr;
	    ...
	    {
	      objinfo.dyn_offset =
		(phdr->p_vaddr + state->elf_loader.base_addr) -
		objinfo.public_rw_ptr;
	      ...
	      for (dt =
		   (unsigned int *) (state->elf_loader.base_addr +
				     phdr->p_vaddr); *dt != DT_NULL; dt += 2)
		if (*dt == DT_PLTGOT)
		  {
		    objinfo.got_offset =
		      (*(dt + 1) + state->elf_loader.base_addr) -
		      objinfo.public_rw_ptr;
		    break;
		  }
  ...
  som_object *object;
  if ((err =
       som_register_sharedobject (state->elf_loader_handle, &objinfo,
				  &object)) != NULL)
    goto error;
...
register_client (runcom_state * state)
{
  ...
  while (phnum--)
	...
	{
	  objinfo.dyn_offset =
	    (som_PTR) phdr->p_vaddr - objinfo.public_rw_ptr;
	  objinfo.dyn_size = phdr->p_memsz;
	  ...
	  for (dt = (unsigned int *) phdr->p_vaddr; *dt != DT_NULL; dt += 2)
	    if (*dt == DT_PLTGOT)
	      {
		objinfo.got_offset =
		  (som_PTR) * (dt + 1) - objinfo.public_rw_ptr;
		break;
	      }
	}
...
load_dynamic_loader (runcom_state * state)
{
  ...
  if ((err =
       som_alloc (strlen (loader_path) +
		  strlen (state->elf_prog.interp_name) + 1,
  ...
  if ((err =
       som_alloc_lib (state->elf_loader.memory_size,
		      (void **) (void *) &state->elf_loader.base_addr)) ==
      NULL)
    err =
      elffile_load (&state->elf_loader, state->elf_loader.base_addr, false);
...
#define NUM_ARGV_ARRAY_ENTRIES 3	/* argc + program name + terminating NULL entry.  */
  ...
  if (state->free_memory + strlen (state->elf_prog_name) + 1 >=
      (som_PTR) state->env.ram_limit)
    return somerr_wimpslot_too_small;
  state->free_memory =
    word_align (state->free_memory + strlen (state->elf_prog_name) + 1);
...
command_run (const char *arg_string, int argc)
{
  ...
  if ((err =
       som_alloc (sizeof (runcom_state), (void **) (void *) &state)) != NULL)
    return err;
  ...
  if ((err =
       som_alloc (dde_cl_len + strlen (arg_string) + 2,
  ...
  state->free_memory =
    state->elf_prog.base_addr + state->elf_prog.memory_size;

  /* Check that loading the executable won't overwrite the stack.  */
  if (state->free_memory >=
      (som_PTR) (state->env.ram_limit - SOM_RUN_STACK_SIZE))
    {
  ...
  if (state->elf_prog.dynamic_seg == NULL)
  ...
  else
    {
      ...
      state->elf_loader_handle =
	som_handle_from_name (state->elf_prog.interp_name);
  ...
  if ((err =
       os_start_app (state->ddeutils_is_present ? "" : state->elf_prog_args,
