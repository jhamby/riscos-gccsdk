...
module_initialise (const char *tail, int podule_base, void *pw)
{
  ...
  if ((err =
       dynamic_area_create ("SO Manager", SOM_INIT_DA_SIZE, SOM_MAX_DA_SIZE,
			    &global.data_da)) != NULL)
      goto error;
  ...
  if (global.flags.host_32bit)
    {
      if ((err =
	   dynamic_area_create ("DSO Libraries", SOM_INIT_LIBDA_SIZE,
				SOM_MAX_LIBDA_SIZE,
				&global.library_da)) != NULL)
	  goto error;

      if ((err =
	   heap_init (global.library_da.base_addr,
  ...
  if ((err =
       somarray_init (&global.object_array, sizeof (som_object *),
		      1)) != NULL)
      goto error;
