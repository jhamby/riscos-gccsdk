...
static int
accept (int sockfd, struct sockaddr *addr, size_t * addrlen)
{
  int child;
  _kernel_oserror *e;

  e = _swix (Socket_Accept, _INR (0, 2) | _OUT (0),
	     sockfd, addr, addrlen, &child);
  if (e != NULL)
    {
...
static int
bind (int sockfd, const struct sockaddr *addr, size_t addrlen)
{
  int error;
  _kernel_oserror *e;

  e = _swix (Socket_Bind, _INR (0, 2) | _OUT (0),
	     sockfd, addr, addrlen, &error);
  if (e != NULL)
    {
...
static int
connect (int sockfd, const struct sockaddr *serv_addr, size_t addrlen)
{
  int error;
  _kernel_oserror *e;

  e = _swix (Socket_Connect, _INR (0, 2) | _OUT (0),
	     sockfd, serv_addr, addrlen, &error);
  if (e != NULL)
    {
...
gethostbyname (const char *name)
{
  ...
  e = _swix (Internet_GetHostByName,
	     _INR (0, 1) | _OUTR (0, 1), 0, name, &error, &h);
...
static int
ioctl (int s, int request, void *data)
{
  int error;
  _kernel_oserror *e;

  e = _swix (Socket_Ioctl, _INR (0, 2) | _OUT (0), s, request, data, &error);
  if (e != NULL)
    {
...
listen (int sockfd, int backlog)
{
  ...
  e = _swix (Socket_Listen, _INR (0, 1) | _OUT (0), sockfd, backlog, &error);
  if (e != NULL)
    {
...
recv (int s, void *buf, size_t len, int flags)
{
  ...
  e = _swix (Socket_Read, _INR (0, 3) | _OUT (0), s, buf, len, flags, &read);
  if (e != NULL)
    {
...
send (int s, const void *buf, size_t len, int flags)
{
  ...
  e = _swix (Socket_Send, _INR (0, 3) | _OUT (0), s, buf, len, flags, &sent);
  if (e != NULL)
    {
...
shutdown (int s, int how)
{
  ...
  e = _swix (Socket_Shutdown, _INR (0, 1) | _OUT (0), s, how, &error);
  if (e != NULL)
    {
...
static int
socketclose (int sock)
{
  int error;
  _kernel_oserror *e;

  e = _swix (Socket_Close, _IN (0) | _OUT (0), sock, &error);
  if (e != NULL)
    {
