...
static uint32_t step_ldr_str (uint32_t instruction,
			      const cpu_registers * regs);
static uint32_t step_ldm_stm (uint32_t instruction,
			      const cpu_registers * regs);
static uint32_t step_branch (uint32_t instruction,
			     const cpu_registers * regs);
 ...
 */
uint32_t
step_instruction (uint32_t instruction, const cpu_registers * regs)
{
  enum
  {
  ...
  if (cond < HI)
  ...
  else if (cond < AL)
    {
      ...
      execute = cond == GT ? (z == 0 && nv == 0) : (z != 0 || nv != 0);
    }
  else
    {
      /* AL, (NV) */
...
static uint32_t
step_nv (uint32_t instruction, const cpu_registers * regs)
{
...
static uint32_t
step_data_processing (uint32_t instruction, const cpu_registers * regs)
{
...
static uint32_t
step_misc_non_dp (uint32_t instruction, const cpu_registers * regs)
{
...
static uint32_t
step_mul_ld_st (uint32_t instruction __attribute__ ((unused)),
		const cpu_registers * regs)
{
...
static uint32_t
step_ldr_str (uint32_t instruction, const cpu_registers * regs)
{
...
static uint32_t
step_ldm_stm (uint32_t instruction, const cpu_registers * regs)
{
  ...
  /** \todo this may abort */
  return *(const uint32_t *)address & ~0x1;
}

static uint32_t
step_branch (uint32_t instruction, const cpu_registers * regs)
{
...
static uint32_t
step_coprocessor (uint32_t instruction __attribute__ ((unused)),
		  const cpu_registers * regs)
{
...
static uint32_t
shift_value (uint32_t value, uint32_t type, uint32_t amount,
	     uint32_t carry, uint32_t reg)
{
  enum
